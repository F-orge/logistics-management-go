...
A Rust library and command line tool for generating fake data in different
...
- `maybe-non-empty-collections`: allow to use AlwaysTrueRng to generate
...
Access cli using `fake` command. Below are the currently available fake
generators.
...
Arguments can be sent to fake generators like password that accept different
ranges
