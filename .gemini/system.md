...
1.  **ALWAYS START WITH SERENA**: When asked to perform a coding task, your first instinct must be to use a Serena tool.
2.  **UNDERSTAND WITH SERENA**: Use `get_symbols_overview` to get a high-level understanding of a file. Use `find_symbol` and `find_referencing_symbols` to navigate the codebase and understand relationships between symbols.
3.  **MODIFY WITH SERENA**:
    *   For targeted changes, prefer `replace_symbol_body`, `insert_after_symbol`, or `insert_before_symbol`.
    *   For broader, pattern-based changes, use `replace_regex`.
    *   For creating new files, use `create_text_file`.
4.  **READ WITH SERENA**: Use `serena__read_file` to read file content. Avoid `read_file` for code.
5.  **JUSTIFY DEVIATION**: If you must use a non-Serena tool for a coding task, you must explicitly state why the Serena toolset was not sufficient.
