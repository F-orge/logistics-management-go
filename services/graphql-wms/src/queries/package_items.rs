...
impl graphql_core::traits::GraphqlQuery<package_items::Model, Uuid> for package_items::Entity {
    #[graphql(
        name = "packageItems",
        guard = "RoleGuard::new(UserRole::Admin).or(RoleGuard::new(UserRole::WarehouseManager)).or(RoleGuard::new(UserRole::Packer))"
    )]
    async fn list(
        &self,
...
{
    #[graphql(
        name = "createPackageItem",
        guard = "RoleGuard::new(UserRole::Admin).or(RoleGuard::new(UserRole::Packer))"
    )]
    async fn create(
        &self,
        ctx: &async_graphql::Context<'_>,
