...
impl graphql_core::traits::GraphqlQuery<rate_rules::Model, Uuid> for rate_rules::Entity {
    ...
    async fn list(&self, ctx: &async_graphql::Context<'_>) -> async_graphql::Result<Vec<rate_rules::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
        let items = rate_rules::Entity::find().all(db).await.unwrap_or_default();
        Ok(items)
    }
    #[graphql(name = "rateRule")]
    async fn view(&self, ctx: &async_graphql::Context<'_>, id: Uuid) -> async_graphql::Result<Option<rate_rules::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
...
impl graphql_core::traits::GraphqlMutation<rate_rules::Model, Uuid, InsertRateRule, UpdateRateRule> for Mutations {
    #[graphql(name = "createRateRule")]
    async fn create(&self, ctx: &async_graphql::Context<'_>, value: InsertRateRule) -> async_graphql::Result<rate_rules::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
    ...
    async fn update(&self, ctx: &async_graphql::Context<'_>, id: Uuid, value: UpdateRateRule) -> async_graphql::Result<rate_rules::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
    ...
    async fn delete(&self, ctx: &async_graphql::Context<'_>, id: Uuid) -> async_graphql::Result<bool> {
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
        let item = rate_rules::Entity::find_by_id(id).one(&trx).await?.ok_or(async_graphql::Error::new("Unable to find rate_rule"))?;
