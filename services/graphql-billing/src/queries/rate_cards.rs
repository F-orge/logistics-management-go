...
impl graphql_core::traits::GraphqlQuery<rate_cards::Model, Uuid> for rate_cards::Entity {
    ...
    async fn list(&self, ctx: &async_graphql::Context<'_>) -> async_graphql::Result<Vec<rate_cards::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
        let items = rate_cards::Entity::find().all(db).await.unwrap_or_default();
        Ok(items)
    }
    #[graphql(name = "rateCard")]
    async fn view(&self, ctx: &async_graphql::Context<'_>, id: Uuid) -> async_graphql::Result<Option<rate_cards::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
...
impl graphql_core::traits::GraphqlMutation<rate_cards::Model, Uuid, InsertRateCard, UpdateRateCard> for Mutations {
    #[graphql(name = "createRateCard")]
    async fn create(&self, ctx: &async_graphql::Context<'_>, value: InsertRateCard) -> async_graphql::Result<rate_cards::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
    ...
    async fn update(&self, ctx: &async_graphql::Context<'_>, id: Uuid, value: UpdateRateCard) -> async_graphql::Result<rate_cards::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
    ...
    async fn delete(&self, ctx: &async_graphql::Context<'_>, id: Uuid) -> async_graphql::Result<bool> {
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
        let item = rate_cards::Entity::find_by_id(id).one(&trx).await?.ok_or(async_graphql::Error::new("Unable to find rate_card"))?;
