    ...
    for driver_locations::Entity
{
    ...
    async fn list(
        ...
        let items = driver_locations::Entity::find()
            .paginate(db, limit)
            .fetch_page(page)
            .await
            .unwrap_or_default();
    ...
    > for Mutations
{
    ...
    ) -> async_graphql::Result<driver_locations::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
    ...
    async fn delete(
        ...
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
        let item = driver_locations::Entity::find_by_id(id)
            .one(&trx)
            .await?
            .ok_or(async_graphql::Error::new("Unable to find driver_location"))?;
        let result = item.delete(&trx).await?;
        _ = trx.commit().await?;
        if result.rows_affected != 1 {
            return Err(async_graphql::Error::new(
                "Unable to delete driver_location",
            ));
        }
        Ok(true)
    }
