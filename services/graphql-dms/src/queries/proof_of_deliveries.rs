    ...
    for proof_of_deliveries::Entity
{
    ...
    async fn view(
        ...
        let db = ctx.data::<DatabaseConnection>()?;
        let item = proof_of_deliveries::Entity::find_by_id(id).one(db).await?;
        Ok(item)
    }
}
...
#[Object(name = "DmsProofOfDeliveryMutations")]
impl
    ...
    > for Mutations
{
    ...
    async fn delete(
        ...
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
        let item = proof_of_deliveries::Entity::find_by_id(id)
            .one(&trx)
            .await?
            .ok_or(async_graphql::Error::new(
                "Unable to find proof_of_delivery",
            ))?;
        let result = item.delete(&trx).await?;
        _ = trx.commit().await?;
        if result.rows_affected != 1 {
            return Err(async_graphql::Error::new(
                "Unable to delete proof_of_delivery",
            ));
        }
        Ok(true)
    }
