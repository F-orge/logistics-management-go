...
use uuid::Uuid;
use crate::entities::{_generated::routes, routes::{InsertRoute, UpdateRoute}};

#[Object(name = "Routes")]
impl graphql_core::traits::GraphqlQuery<routes::Model, Uuid> for routes::Entity {
    #[graphql(name = "routes")]
    async fn list(&self, ctx: &async_graphql::Context<'_>) -> async_graphql::Result<Vec<routes::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
        let routes = routes::Entity::find().all(db).await.unwrap_or_default();
        Ok(routes)
    }
    #[graphql(name = "route")]
    async fn view(&self, ctx: &async_graphql::Context<'_>, id: Uuid) -> async_graphql::Result<Option<routes::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
...
impl graphql_core::traits::GraphqlMutation<routes::Model, Uuid, InsertRoute, UpdateRoute> for Mutations {
    #[graphql(name = "createRoute")]
    async fn create(&self, ctx: &async_graphql::Context<'_>, value: InsertRoute) -> async_graphql::Result<routes::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
    ...
    async fn update(&self, ctx: &async_graphql::Context<'_>, id: Uuid, value: UpdateRoute) -> async_graphql::Result<routes::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
    ...
    async fn delete(&self, ctx: &async_graphql::Context<'_>, id: Uuid) -> async_graphql::Result<bool> {
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
        let route = routes::Entity::find_by_id(id).one(&trx).await?.ok_or(async_graphql::Error::new("Unable to find route"))?;
