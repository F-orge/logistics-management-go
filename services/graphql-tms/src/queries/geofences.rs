...
use uuid::Uuid;
use crate::entities::{_generated::geofences, geofences::{InsertGeofence, UpdateGeofence}};
...
impl graphql_core::traits::GraphqlQuery<geofences::Model, Uuid> for geofences::Entity {
    ...
    async fn list(&self, ctx: &async_graphql::Context<'_>) -> async_graphql::Result<Vec<geofences::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
        let geofences = geofences::Entity::find().all(db).await.unwrap_or_default();
        Ok(geofences)
    }
    #[graphql(name = "geofence")]
    async fn view(&self, ctx: &async_graphql::Context<'_>, id: Uuid) -> async_graphql::Result<Option<geofences::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
...
impl graphql_core::traits::GraphqlMutation<geofences::Model, Uuid, InsertGeofence, UpdateGeofence> for Mutations {
    #[graphql(name = "createGeofence")]
    async fn create(&self, ctx: &async_graphql::Context<'_>, value: InsertGeofence) -> async_graphql::Result<geofences::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
    ...
    async fn update(&self, ctx: &async_graphql::Context<'_>, id: Uuid, value: UpdateGeofence) -> async_graphql::Result<geofences::Model> {
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
    ...
    async fn delete(&self, ctx: &async_graphql::Context<'_>, id: Uuid) -> async_graphql::Result<bool> {
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
        let geofence = geofences::Entity::find_by_id(id).one(&trx).await?.ok_or(async_graphql::Error::new("Unable to find geofence"))?;
