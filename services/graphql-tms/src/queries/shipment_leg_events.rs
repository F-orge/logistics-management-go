use crate::entities::{
    _generated::shipment_leg_events,
    ...
    for shipment_leg_events::Entity
{
    ...
    async fn list(
        ...
        let shipment_leg_events = shipment_leg_events::Entity::find()
            .paginate(db, limit)
            .fetch_page(page)
            .await
            .unwrap_or_default();
    ...
    async fn view(
        ...
        ctx: &async_graphql::Context<'_>,
        id: Uuid,
    ) -> async_graphql::Result<Option<shipment_leg_events::Model>> {
        let db = ctx.data::<DatabaseConnection>()?;
...
impl
    graphql_core::traits::GraphqlMutation<
        shipment_leg_events::Model,
        Uuid,
        InsertShipmentLegEvent,
        UpdateShipmentLegEvent,
    > for Mutations
{
    #[graphql(
        name = "createShipmentLegEvent",
    ...
    async fn delete(
        ...
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
        let shipment_leg_event = shipment_leg_events::Entity::find_by_id(id)
            .one(&trx)
            .await?
            .ok_or(async_graphql::Error::new(
                "Unable to find shipment leg event",
            ))?;
        let result = shipment_leg_event.delete(&trx).await?;
        _ = trx.commit().await?;
        if result.rows_affected != 1 {
            return Err(async_graphql::Error::new(
                "Unable to delete shipment leg event",
            ));
        }
        Ok(true)
    }
