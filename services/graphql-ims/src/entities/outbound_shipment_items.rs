...
use crate::entities::_generated::{
    inventory_batches, outbound_shipments, products, sales_order_items,
};
...
impl outbound_shipment_items::Model {
    async fn outbound_shipment(
        ...
        let result = outbound_shipments::Entity::find_by_id(self.outbound_shipment_id)
            .one(db)
            .await?;
        match result {
            Some(model) => Ok(model),
    ...
    async fn sales_order_item(
        ...
        let db = ctx.data::<DatabaseConnection>()?;
        let result = sales_order_items::Entity::find_by_id(self.sales_order_item_id)
            .one(db)
            .await?;
        match result {
            Some(model) => Ok(model),
