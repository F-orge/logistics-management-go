    ...
    for inbound_shipments::Entity
{
    #[graphql(
        ...
        guard = "RoleGuard::new(UserRole::Admin).or(RoleGuard::new(UserRole::InventoryManager)).or(RoleGuard::new(UserRole::WarehouseManager)).or(RoleGuard::new(UserRole::WarehouseOperator)).or(RoleGuard::new(UserRole::ReceivingManager)).or(RoleGuard::new(UserRole::AccountManager))"
    )]
    async fn list(
        &self,
    ...
    > for Mutations
{
    #[graphql(
        ...
        guard = "RoleGuard::new(UserRole::Admin).or(RoleGuard::new(UserRole::ReceivingManager)).or(RoleGuard::new(UserRole::WarehouseManager))"
    )]
    async fn create(
        &self,
        ctx: &async_graphql::Context<'_>,
