...
impl graphql_core::traits::GraphqlQuery<reorder_points::Model, Uuid> for reorder_points::Entity {
    #[graphql(
        name = "reorderPoints",
        guard = "RoleGuard::new(UserRole::Admin).or(RoleGuard::new(UserRole::InventoryManager)).or(RoleGuard::new(UserRole::AccountManager))"
    )]
    async fn list(
        &self,
