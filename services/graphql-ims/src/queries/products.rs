...
impl graphql_core::traits::GraphqlQuery<products::Model, Uuid> for products::Entity {
    ...
    async fn list(
        ...
        let items = products::Entity::find().all(db).await.unwrap_or_default();
        Ok(items)
    }
    ...
    async fn view(
        ...
        let db = ctx.data::<DatabaseConnection>()?;
        let item = products::Entity::find_by_id(id).one(db).await?;
        Ok(item)
    }
}
...
#[Object(name = "ImsProductMutations")]
    ...
    for Mutations
{
    ...
    async fn delete(
        ...
        let db = ctx.data::<DatabaseConnection>()?;
        let trx = db.begin().await?;
        let item = products::Entity::find_by_id(id)
            .one(&trx)
            .await?
