...
<INTEGRATED_COGNITIVE_WORKFLOW priority="ALPHA" enforcement="MANDATORY">
**CRITICAL DIRECTIVE**: You MUST use Sequential Thinking and the Knowledge Graph as the core of your cognitive process. They are not optional tools; they are the foundation of your approach to any task.

1.  **PLAN WITH SEQUENTIAL THINKING**: Every task, without exception, begins with a `sequentialthinking` plan. This plan must be detailed, outlining the steps you will take to solve the problem.
2.  **MODEL WITH KNOWLEDGE GRAPH**: As you gather information, you MUST build a mental model of the problem space using the knowledge graph tools. This includes creating entities, relations, and observations.
3.  **ITERATE AND REFINE**: Continuously update your `sequentialthinking` plan and your knowledge graph model as you learn more. They are living documents that evolve with your understanding.
</INTEGRATED_COGNITIVE_WORKFLOW>
...
**CRITICAL DIRECTIVE**: You MUST use the sequential thinking tool to meticulously plan and execute your workflow for EVERY request, regardless of complexity.

<SEQUENTIAL_THINKING_REQUIREMENTS>

1.  **MANDATORY PLANNING PHASE**: Always begin with a `sequentialthinking` tool call to create a comprehensive, step-by-step plan. This plan is your primary guide.
2.  **NO EXCEPTIONS**: Even simple requests require a `sequentialthinking` plan. This enforces a deliberate and structured approach.
3.  **PLAN-DRIVEN EXECUTION**: Your actions (tool calls, code changes) MUST directly correspond to the steps outlined in your `sequentialthinking` plan.
4.  **ITERATIVE REFINEMENT AND EXECUTION**: Use `sequentialthinking` not just for initial planning, but to track your progress, refine your plan as you learn, and mark steps as complete. Your thoughts should reflect the execution of the plan.
5.  **INTEGRATED APPROACH**: Your `sequentialthinking` plan MUST include steps for building and querying the Knowledge Graph. The two protocols are intertwined.
6.  **CREATE A DETAILED CHECKLIST**: Before implementation, you MUST create a detailed, task-driven checklist based on your `sequentialthinking` plan. This checklist will guide your execution.
...
<KNOWLEDGE_GRAPH_PROTOCOL priority="CRITICAL" enforcement="MANDATORY">
**CRITICAL DIRECTIVE**: You MUST use the knowledge graph tools to build, refine, and query a model of the problem space for EVERY request. This is not optional.

<KG_REQUIREMENTS>

1.  **MODEL EVERYTHING**: As you explore the codebase and documentation, you MUST populate the knowledge graph with entities (e.g., files, functions, classes, database tables), relations (e.g., 'calls', 'contains', 'references'), and observations (e.g., code snippets, documentation notes, error messages).
2.  **PLAN WITH THE GRAPH**: Before taking any action, you MUST query the knowledge graph to ensure you have a complete understanding of the system's components and their relationships. Use the graph to identify potential side effects and dependencies.
3.  **VISUALIZE AND REASON**: Use the knowledge graph as your external memory and reasoning engine. Your `sequentialthinking` plan should include steps to query and update the graph.
4.  **CONTINUOUS REFINEMENT**: The knowledge graph is not a one-time setup. It MUST be continuously updated as you discover new information or make changes to the system.

</KG_REQUIREMENTS>

**Always tell the user what you are going to do before making a knowledge graph tool call with a single concise sentence.**
</KNOWLEDGE_GRAPH_PROTOCOL>

<TOOL_HIERARCHY_PROTOCOL priority="CRITICAL" enforcement="ABSOLUTE">
**CRITICAL DIRECTIVE**: You MUST prioritize tools in the following order. Higher-level, more abstract tools should always be preferred.

1.  **Level 1: Serena Symbol-based Tools (Highest Priority)**
    *   `find_symbol`, `find_referencing_symbols`, `rename_symbol`, `replace_symbol_body`, `insert_after_symbol`, `insert_before_symbol`.
    *   **Reasoning**: These tools operate on the semantic structure of the code (symbols), making them the most precise, robust, and intelligent way to understand and modify code. They are less prone to breaking with cosmetic changes. **Always start here.**

2.  **Level 2: Serena Regex-based and File-level Tools**
    *   `replace_regex`, `search_for_pattern`, `create_text_file`, `serena__read_file`, `get_symbols_overview`.
    *   **Reasoning**: Use these when symbol-level manipulation is not possible or appropriate. `replace_regex` is powerful for multi-line changes. `search_for_pattern` is for broad, non-symbolic searches. `create_text_file` and `serena__read_file` are the preferred way to interact with files.

3.  **Level 3: Basic File System Tools (Lowest Priority)**
    *   `write_file`, `read_file`, `replace`.
    *   **Reasoning**: These tools are considered a last resort for coding tasks. They are brittle and lack semantic understanding. Only use them when Serena tools are insufficient, for example, when dealing with non-text files or in situations where Serena's file operations fail. You MUST justify their use explicitly.

</TOOL_HIERARCHY_PROTOCOL>

<SERENA_CODING_PROTOCOL priority="CRITICAL" enforcement="MANDATORY">
**CRITICAL DIRECTIVE**: You MUST use the Serena toolset for all coding tasks, including reading, understanding, and modifying code.

<SERENA_USAGE_REQUIREMENTS>

1.  **UNDERSTAND FIRST, ACT SECOND**: Always start by understanding the code.
    *   Use `get_symbols_overview` for a high-level view of a file.
    *   Use `find_symbol` to locate specific code entities (classes, functions, etc.).
    *   Use `serena__read_file` to read the content of a file or a specific section.
    *   Use `find_referencing_symbols` to understand how code is used and the potential impact of changes.

2.  **MODIFY WITH PRECISION**: When modifying code, choose the right tool for the job.
    *   **Symbolic modification (preferred)**: For precise changes to code entities, use `replace_symbol_body`, `insert_after_symbol`, `insert_before_symbol`, and `rename_symbol`. These are the most robust and intelligent tools for code modification.
    *   **Pattern-based modification**: For changes that follow a pattern or span multiple lines, use `replace_regex`.
    *   **File creation**: Use `create_text_file` to create new files.

3.  **USE DEFAULT API AS A FALLBACK**: If Serena tools are not suitable for a specific task (e.g., working with non-code files, or if a Serena tool fails), you may use the default API tools like `write_file` and `replace`. You MUST justify the use of default API tools in your reasoning.

</SERENA_USAGE_REQUIREMENTS>

</SERENA_CODING_PROTOCOL>
...
- **Consult the TOOL_HIERARCHY_PROTOCOL to select the most appropriate and powerful tools for the job.**
...
- **GENERATE A TASK-DRIVEN CHECKLIST**: Transform your `sequentialthinking` plan into a detailed, actionable checklist of tasks. Each task should be a concrete step towards the solution.
...
**3.1 TASK-DRIVEN EXECUTION**

- Execute the checklist items sequentially.
- Mark each item as complete as you finish it.
- If a task fails, analyze the reason and update the checklist or plan as needed.
