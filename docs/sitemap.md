...
* `/forgot-password`
* `/reset-password`
* Optional: `/register` (If self-registration for certain roles like `customer_rep` is allowed, though `users` create rule is `it_admin` only)
...
## III. User & Company Management (Primarily for IT Admin, Executive, specific managers)
* **A. My Profile**
    * `/profile` (View own user details)
    * `/profile/edit` (Edit own user details - name, avatar, phone, password)
* **B. User Management (IT Admin)**
    * `/admin/users` (List all users - `users` collection)
    * `/admin/users/new` (Create new user)
    * `/admin/users/{user_id}` (View user details)
    * `/admin/users/{user_id}/edit` (Edit user details - role, company, department, etc.)
* **C. Company Management (IT Admin, Executive, Customer Service Rep)**
...
* **D. Department Management (Executive, IT Admin, Department Managers)**
...
* **A. Product Catalog (Warehouse Manager, IT Admin, Executive, and view for many others)**
...
* **B. Inventory Management (Warehouse Manager, Inventory Clerk, Executive, IT Admin)**
...
* **A. Orders (Customer Service Rep, Warehouse Manager, Dispatch Coordinator, Finance, Executive, IT Admin, Customer Rep for their own orders)**
...
* **A. Warehouse Management (Executive, IT Admin, Warehouse Manager for their own)**
...
* **C. Vehicle Management (Dispatch Coordinator, Executive, IT Admin)**
...
## VIII. Financial Management (Finance Dept, Executive, IT Admin, Customer Rep for their own invoices)
* **A. Invoices**
    * `/invoices` (List all invoices - `invoices` collection, filterable by customer, status, date)
    * `/invoices/new` (Create new invoice - often generated from an order)
    * `/invoices/{invoice_number}` (View invoice details)
        ...
        * Related Payments
    * `/invoices/{invoice_number}/edit` (Edit invoice details - status, if permissible)
    * `/invoices/{invoice_number}/send` (Action to mark as sent)
    * `/invoices/{invoice_number}/record-payment` (Navigate to create payment)
* **B. Payments**
    * `/payments` (List all payments - `payments` collection, filterable by invoice, date, method)
    * `/payments/new` (Record new payment against an invoice)
    * `/payments/{payment_id}` (View payment details - date, amount, method, transaction ID, notes)
    * `/payments/{payment_id}/edit` (Edit payment details - status, notes, if permissible)
...
## X. Settings / System Configuration (Primarily IT Admin, Executive)
* `/settings` (General application settings, if any)
* `/settings/roles-permissions` (Conceptual - managing roles beyond PocketBase's basic structure, if needed)
* `/settings/integrations` (Conceptual - for external system connections)