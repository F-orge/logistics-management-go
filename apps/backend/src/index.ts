...
import * as orpcRouter from '@packages/rpc/src/handlers'
...
export const serverFactory = async ({ pool }: ServerFactory) => {
  ...
  for (const migration of migrationResult.results || []) {
    console.info(migration.migrationName, migration.direction, migration.status)
  }
  ...
  // better auth
  const auth = authFactory(pool, process.env.NODE_ENV === 'production' ? sgMail : localMailer, true)
