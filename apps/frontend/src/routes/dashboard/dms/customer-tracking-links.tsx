import { columns } from "@/components/tables/dms/customer_tracking_links";
import {
  NewCustomerTrackingLinkDialogForm,
  UpdateCustomerTrackingLinkDialogForm,
} from "@/components/forms/dms/customer_tracking_links";
import { execute, CustomerTrackingLinkQuery } from "@packages/graphql/client";
import {
  Button,
  DataTable,
  InputGroup,
...
export const Route = createFileRoute("/dashboard/dms/customer-tracking-links")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", CustomerTrackingLinkQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.dms?.customerTrackingLinks || [];
  },
});
