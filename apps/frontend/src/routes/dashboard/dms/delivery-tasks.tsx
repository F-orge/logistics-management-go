...
export const Route = createFileRoute("/dashboard/dms/delivery-tasks")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableDeliveryTaskQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
      status: context.search.status,
      failureReason: context.search.failureReason,
    });

    return result.data?.dms?.deliveryTasks || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(DeliveryTaskStatus).map((item) => ({
            label: item,
            value: item,
          }))}
        ...
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, failureReason: v as any }),
            })
          }
          options={Object.values(DeliveryFailureReason).map((item) => ({
            label: item,
            value: item,
          }))}
