import { columns } from "@/components/tables/tms/vehicles";
import {
  VehicleStatus,
  execute,
  TableVehicleQuery,
...
export const Route = createFileRoute("/dashboard/tms/vehicles")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableVehicleQuery, {
    ...
    return result.data?.tms?.vehicles || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(VehicleStatus).map((item) => ({
            label: item,
            value: item,
          }))}
