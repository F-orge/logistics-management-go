import { columns } from "@/components/tables/tms/trips";
import { TripStatus, execute, TableTripQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/tms/trips")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableTripQuery, {
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(TripStatus).map((item) => ({
            label: item,
            value: item,
          }))}
