import { columns } from "@/components/tables/tms/routes";
import { execute, TableRouteQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/tms/routes")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableRouteQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.tms?.routes || [];
  },
});
