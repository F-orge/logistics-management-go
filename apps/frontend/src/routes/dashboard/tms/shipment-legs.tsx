import { columns } from "@/components/tables/tms/shipment_legs";
import {
  ShipmentLegStatus,
  execute,
...
export const Route = createFileRoute("/dashboard/tms/shipment-legs")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      status: z.enum(ShipmentLegStatus).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableShipmentLegQuery, {
    ...
    return result.data?.tms?.shipmentLegs || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(ShipmentLegStatus).map((item) => ({
            label: item,
            value: item,
          }))}
