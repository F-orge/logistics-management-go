import { columns } from "@/components/tables/tms/carriers";
import { execute, TableCarrierQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/tms/carriers")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableCarrierQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.tms?.carriers || [];
  },
});
