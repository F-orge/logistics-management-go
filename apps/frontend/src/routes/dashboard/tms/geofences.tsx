import { columns } from "@/components/tables/tms/geofences";
import { execute, TableGeofenceQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/tms/geofences")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableGeofenceQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.tms?.geofences || [];
  },
});
