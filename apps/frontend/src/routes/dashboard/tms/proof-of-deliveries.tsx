import { columns } from "@/components/tables/tms/proof_of_deliveries";
import {
  ProofType,
  execute,
...
export const Route = createFileRoute("/dashboard/tms/proof-of-deliveries")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      type: z.enum(ProofType).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableTmsProofOfDeliveryQuery, {
    ...
    return result.data?.tms?.proofOfDeliveries || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(ProofType).map((item) => ({
            label: item,
            value: item,
          }))}
