import { columns } from "@/components/tables/tms/drivers";
import {
  DriverStatus,
  execute,
  TableDriverQuery,
...
export const Route = createFileRoute("/dashboard/tms/drivers")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableDriverQuery, {
    ...
    return result.data?.tms?.drivers || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(DriverStatus).map((item) => ({
            label: item,
            value: item,
          }))}
