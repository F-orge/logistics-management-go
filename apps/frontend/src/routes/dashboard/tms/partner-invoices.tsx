import { columns } from "@/components/tables/tms/partner_invoices";
import {
...
export const Route = createFileRoute("/dashboard/tms/partner-invoices")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      status: z.enum(PartnerInvoiceStatus).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TablePartnerInvoice, {
    ...
    return result.data?.tms?.partnerInvoices || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(PartnerInvoiceStatus).map((item) => ({
            label: item,
            value: item,
          }))}
