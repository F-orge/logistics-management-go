...
import * as contract from "@packages/rpc/src/contracts/crm/contacts";
...
const columns = generateColumn(
  contract.PaginateContactContract["~orpc"].outputSchema?.unwrap()
);

export const Route = createFileRoute("/dashboard/crm/contacts")({
  component: RouteComponent,
  validateSearch: zodValidator(
    contract.PaginateContactContract["~orpc"].inputSchema!.extend({
  ...
  loader: async ({ context }) => {
    return context.queryClient.fetchQuery(
      orpcQueryOption("crm", "PaginateContact", context.search)
    );
  },
});
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    orpcMutationOption("crm", "InsertContact")
  );

  const updateMutation = useMutation(
    orpcMutationOption("crm", "UpdateContact")
  );

  const deleteMutation = useMutation(
    orpcMutationOption("crm", "RemoveContact")
  );
  ...
  return (
    <article className="grid grid-cols-12 gap-2.5">
      ...
      <section>
        <AutoFormDialog
          schema={contract.InsertContactContract["~orpc"].inputSchema!}
        ...
        <AutoFormDialog
          schema={
            contract.UpdateContactContract["~orpc"].inputSchema!.shape.value
          }
            ...
            updateMutation.mutateAsync(
              { id: searchQuery.id as string, value },
              {
                onSuccess: () => {
                  ...
                  toast.success("Contact record has been updated");
                },
              }
            )
          }
