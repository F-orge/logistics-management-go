import { columns } from "@/components/tables/crm/contacts";
import {
  NewContactDialogForm,
  UpdateContactDialogForm,
} from "@/components/forms/crm/contacts";
import { execute, TableContactQuery } from "@packages/graphql/client";
import {
  Button,
  DataTable,
  InputGroup,
...
export const Route = createFileRoute("/dashboard/crm/contacts")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableContactQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.crm?.contacts || [];
  },
});
...
function RouteComponent() {
  ...
  return (
    <article className="grid grid-cols-12 gap-5">
      ...
      <section>
        <NewContactDialogForm />
        <UpdateContactDialogForm data={data} />
      </section>
    </article>
  );
}
