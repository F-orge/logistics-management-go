...
import * as contract from "@packages/rpc/src/contracts/crm/cases";
...
const columns = generateColumn(
  contract.PaginateCaseContract["~orpc"].outputSchema?.unwrap()
);

export const Route = createFileRoute("/dashboard/crm/cases")({
  component: RouteComponent,
  validateSearch: zodValidator(
    contract.PaginateCaseContract["~orpc"].inputSchema!.extend({
  ...
  loader: async ({ context }) => {
    return context.queryClient.fetchQuery(
      orpcQueryOption("crm", "PaginateCase", context.search)
    );
  },
});
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    orpcMutationOption("crm", "InsertCase")
  );

  const updateMutation = useMutation(
    orpcMutationOption("crm", "UpdateCase")
  );

  const deleteMutation = useMutation(
    orpcMutationOption("crm", "RemoveCase")
  );
  ...
  return (
    <article className="grid grid-cols-12 gap-2.5">
      ...
      <section>
        <AutoFormDialog
          schema={contract.InsertCaseContract["~orpc"].inputSchema!}
          ...
          onSubmit={(value) =>
            createMutation.mutateAsync(value, {
              onSuccess: () => {
                navigate({ search: (prev) => ({ ...prev, new: undefined }) });
                toast.success("New case has been created");
              },
            })
          }
        />
        <AutoFormDialog
          schema={
            contract.UpdateCaseContract["~orpc"].inputSchema!.shape.value
          }
            ...
            updateMutation.mutateAsync(
              { id: searchQuery.id as string, value },
              {
                onSuccess: () => {
                  ...
                  toast.success("Case record has been updated");
                },
              }
            )
          }
