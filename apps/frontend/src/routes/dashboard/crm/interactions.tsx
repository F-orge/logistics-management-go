...
import * as contract from "@packages/rpc/src/contracts/crm/interactions";
...
const columns = generateColumn(
  contract.PaginateInteractionContract["~orpc"].outputSchema?.unwrap()
);

export const Route = createFileRoute("/dashboard/crm/interactions")({
  component: RouteComponent,
  validateSearch: zodValidator(
    contract.PaginateInteractionContract["~orpc"].inputSchema!.extend({
  ...
  loader: async ({ context }) => {
    return context.queryClient.fetchQuery(
      orpcQueryOption("crm", "PaginateInteraction", context.search)
    );
  },
});
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    orpcMutationOption("crm", "InsertInteraction")
  );

  const updateMutation = useMutation(
    orpcMutationOption("crm", "UpdateInteraction")
  );

  const deleteMutation = useMutation(
    orpcMutationOption("crm", "RemoveInteraction")
  );
    ...
    <article className="grid grid-cols-12 gap-2.5">
      ...
      <section className="col-span-full">
        <DataTable
          contextMenus={[
            {
              label: "Edit Interaction Information",
              onClick: (row) =>
                navigate({
                  search: (prev) => ({
                    ...prev,
      ...
      <section>
        <AutoFormDialog
          schema={contract.InsertInteractionContract["~orpc"].inputSchema!}
          ...
          onSubmit={(value) =>
            createMutation.mutateAsync(value, {
              onSuccess: () => {
                navigate({ search: (prev) => ({ ...prev, new: undefined }) });
                toast.success("New interaction has been created");
              },
            })
          }
        />
        <AutoFormDialog
          schema={
            contract.UpdateInteractionContract["~orpc"].inputSchema!.shape.value
          }
            ...
            updateMutation.mutateAsync(
              { id: searchQuery.id as string, value },
              {
                onSuccess: () => {
                  ...
                  toast.success("Interaction record has been updated");
                },
              }
            )
          }
