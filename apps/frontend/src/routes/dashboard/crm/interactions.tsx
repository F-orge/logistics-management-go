import { columns } from "@/components/tables/crm/interactions";
import {
  NewInteractionDialogForm,
  UpdateInteractionDialogForm,
} from "@/components/forms/crm/interactions";
import {
  InteractionType,
  execute,
...
export const Route = createFileRoute("/dashboard/crm/interactions")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableInteractionQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
      interactionType: context.search.interactionType,
    });

    return result.data?.crm?.interactions || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      ...
      <section className="col-span-full flex flex-row items-center gap-2.5">
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, interactionType: v as any }),
            })
          }
          options={Object.values(InteractionType).map((item) => ({
            label: item,
            value: item,
          }))}
      ...
      <section>
        <NewInteractionDialogForm />
        <UpdateInteractionDialogForm data={data} />
      </section>
    </article>
  );
}
