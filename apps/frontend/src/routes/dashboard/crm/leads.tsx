...
import * as contract from "@packages/rpc/src/contracts/crm/leads";
...
const columns = generateColumn(
  contract.PaginateLeadContract["~orpc"].outputSchema?.unwrap()
);

export const Route = createFileRoute("/dashboard/crm/leads")({
  component: RouteComponent,
  validateSearch: zodValidator(
    contract.PaginateLeadContract["~orpc"].inputSchema!.extend({
  ...
  loader: async ({ context }) => {
    return context.queryClient.fetchQuery(
      orpcQueryOption("crm", "PaginateLead", context.search)
    );
  },
});
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    orpcMutationOption("crm", "InsertLead")
  );

  const updateMutation = useMutation(
    orpcMutationOption("crm", "UpdateLead")
  );

  const deleteMutation = useMutation(
    orpcMutationOption("crm", "RemoveLead")
  );
  ...
  return (
    <article className="grid grid-cols-12 gap-2.5">
      ...
      <section>
        <AutoFormDialog
          schema={contract.InsertLeadContract["~orpc"].inputSchema!}
        ...
        <AutoFormDialog
          schema={
            contract.UpdateLeadContract["~orpc"].inputSchema!.shape.value
          }
            ...
            updateMutation.mutateAsync(
              { id: searchQuery.id as string, value },
              {
                onSuccess: () => {
                  ...
                  toast.success("Lead record has been updated");
                },
              }
            )
          }
