import { columns } from "@/components/tables/crm/leads";
import {
  NewLeadDialogForm,
  UpdateLeadDialogForm,
} from "@/components/forms/crm/leads";
import {
  LeadSource,
  LeadStatus,
  execute,
...
export const Route = createFileRoute("/dashboard/crm/leads")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableLeadQuery, {
...
function RouteComponent() {
  ...
  return (
    <article className="grid grid-cols-12 gap-5">
      ...
      <section>
        <NewLeadDialogForm />
        <UpdateLeadDialogForm data={data} />
      </section>
    </article>
  );
}
