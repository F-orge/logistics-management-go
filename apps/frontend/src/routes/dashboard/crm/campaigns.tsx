...
import * as contract from "@packages/rpc/src/contracts/crm/campaigns";
...
const columns = generateColumn(
  contract.PaginateCampaignContract["~orpc"].outputSchema?.unwrap()
);

export const Route = createFileRoute("/dashboard/crm/campaigns")({
  component: RouteComponent,
  validateSearch: zodValidator(
    contract.PaginateCampaignContract["~orpc"].inputSchema!.extend({
  ...
  loader: async ({ context }) => {
    return context.queryClient.fetchQuery(
      orpcQueryOption("crm", "PaginateCampaign", context.search)
    );
  },
});
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    orpcMutationOption("crm", "InsertCampaign")
  );

  const updateMutation = useMutation(
    orpcMutationOption("crm", "UpdateCampaign")
  );

  const deleteMutation = useMutation(
    orpcMutationOption("crm", "RemoveCampaign")
  );
    ...
    <article className="grid grid-cols-12 gap-2.5">
      ...
      <section className="col-span-full">
        <DataTable
          contextMenus={[
            {
              label: "Edit Campaign Information",
              onClick: (row) =>
                navigate({
                  search: (prev) => ({
                    ...prev,
      ...
      <section>
        <AutoFormDialog
          schema={contract.InsertCampaignContract["~orpc"].inputSchema!}
          ...
          onSubmit={(value) =>
            createMutation.mutateAsync(value, {
              onSuccess: () => {
                navigate({ search: (prev) => ({ ...prev, new: undefined }) });
                toast.success("New campaign has been created");
              },
            })
          }
        />
        <AutoFormDialog
          schema={
            contract.UpdateCampaignContract["~orpc"].inputSchema!.shape.value
          }
            ...
            updateMutation.mutateAsync(
              { id: searchQuery.id as string, value },
              {
                onSuccess: () => {
                  ...
                  toast.success("Campaign record has been updated");
                },
              }
            )
          }
