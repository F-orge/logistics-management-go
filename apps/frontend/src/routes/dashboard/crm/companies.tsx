import { columns } from "@/components/tables/crm/companies";
import {
  NewCompanyDialogForm,
  UpdateCompanyDialogForm,
} from "@/components/forms/crm/companies";
import { execute, TableCompanyQuery } from "@packages/graphql/client";
import {
  Button,
  DataTable,
  InputGroup,
...
export const Route = createFileRoute("/dashboard/crm/companies")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableCompanyQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.crm?.companies || [];
  },
});
...
function RouteComponent() {
  ...
  return (
    <article className="grid grid-cols-12 gap-5">
      ...
      <section>
        <NewCompanyDialogForm />
        <UpdateCompanyDialogForm data={data} />
      </section>
    </article>
  );
}
