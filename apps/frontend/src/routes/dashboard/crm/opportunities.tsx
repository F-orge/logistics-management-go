...
import * as contract from "@packages/rpc/src/contracts/crm/opportunities";
...
const columns = generateColumn(
  contract.PaginateOpportunityContract["~orpc"].outputSchema?.unwrap()
);

export const Route = createFileRoute("/dashboard/crm/opportunities")({
  component: RouteComponent,
  validateSearch: zodValidator(
    contract.PaginateOpportunityContract["~orpc"].inputSchema!.extend({
  ...
  loader: async ({ context }) => {
    return context.queryClient.fetchQuery(
      orpcQueryOption("crm", "PaginateOpportunity", context.search)
    );
  },
});
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    orpcMutationOption("crm", "InsertOpportunity")
  );

  const updateMutation = useMutation(
    orpcMutationOption("crm", "UpdateOpportunity")
  );

  const deleteMutation = useMutation(
    orpcMutationOption("crm", "RemoveOpportunity")
  );
    ...
    <article className="grid grid-cols-12 gap-2.5">
      ...
      <section className="col-span-full">
        <DataTable
          contextMenus={[
            {
              label: "Edit Opportunity Information",
              onClick: (row) =>
                navigate({
                  search: (prev) => ({
                    ...prev,
      ...
      <section>
        <AutoFormDialog
          schema={contract.InsertOpportunityContract["~orpc"].inputSchema!}
          ...
          onSubmit={(value) =>
            createMutation.mutateAsync(value, {
              onSuccess: () => {
                navigate({ search: (prev) => ({ ...prev, new: undefined }) });
                toast.success("New opportunity has been created");
              },
            })
          }
        />
        <AutoFormDialog
          schema={
            contract.UpdateOpportunityContract["~orpc"].inputSchema!.shape.value
          }
            ...
            updateMutation.mutateAsync(
              { id: searchQuery.id as string, value },
              {
                onSuccess: () => {
                  ...
                  toast.success("Opportunity record has been updated");
                },
              }
            )
          }
