...
export const Route = createFileRoute("/dashboard/crm/opportunities")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableOpportunityQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
      stage: context.search.stage,
      source: context.search.source,
    });

    return result.data?.crm?.opportunities || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      ...
      <section className="col-span-full flex flex-row items-center gap-2.5">
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, stage: v as any }),
            })
          }
          options={Object.values(OpportunityStage).map((item) => ({
            label: item,
            value: item,
          }))}
        ...
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, source: v as any }),
            })
          }
          options={Object.values(OpportunitySource).map((item) => ({
            label: item,
            value: item,
          }))}
