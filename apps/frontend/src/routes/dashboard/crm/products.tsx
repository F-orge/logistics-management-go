...
import * as contract from "@packages/rpc/src/contracts/crm/products";
...
const columns = generateColumn(
  contract.PaginateProductContract["~orpc"].outputSchema?.unwrap()
);

export const Route = createFileRoute("/dashboard/crm/products")({
  component: RouteComponent,
  validateSearch: zodValidator(
    contract.PaginateProductContract["~orpc"].inputSchema!.extend({
  ...
  loader: async ({ context }) => {
    return context.queryClient.fetchQuery(
      orpcQueryOption("crm", "PaginateProduct", context.search)
    );
  },
});
...
function RouteComponent() {
  ...
  const createMutation = useMutation(
    orpcMutationOption("crm", "InsertProduct")
  );

  const updateMutation = useMutation(
    orpcMutationOption("crm", "UpdateProduct")
  );

  const deleteMutation = useMutation(
    orpcMutationOption("crm", "RemoveProduct")
  );
  ...
  return (
    <article className="grid grid-cols-12 gap-2.5">
      ...
      <section>
        <AutoFormDialog
          schema={contract.InsertProductContract["~orpc"].inputSchema!}
        ...
        <AutoFormDialog
          schema={
            contract.UpdateProductContract["~orpc"].inputSchema!.shape.value
          }
            ...
            updateMutation.mutateAsync(
              { id: searchQuery.id as string, value },
              {
                onSuccess: () => {
                  ...
                  toast.success("Product record has been updated");
                },
              }
            )
          }
