import { columns } from "@/components/tables/billing/surcharges";
import {
  SurchargeCalculationMethod,
  execute,
  TableSurchargeQuery,
...
export const Route = createFileRoute("/dashboard/billing/surcharges")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      calculationMethod: z.enum(SurchargeCalculationMethod).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableSurchargeQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
      calculationMethod: context.search.calculationMethod,
    });

    return result.data?.billing?.surcharges || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, calculationMethod: v as any }),
            })
          }
          options={Object.values(SurchargeCalculationMethod).map((item) => ({
            label: item,
            value: item,
          }))}
