import { columns } from "@/components/tables/billing/disputes";
import {
  DisputeStatus,
  execute,
  TableDisputeQuery,
...
export const Route = createFileRoute("/dashboard/billing/disputes")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableDisputeQuery, {
    ...
    return result.data?.billing?.disputes || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(DisputeStatus).map((item) => ({
            label: item,
            value: item,
          }))}
