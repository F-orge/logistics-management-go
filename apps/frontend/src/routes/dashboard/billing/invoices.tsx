import { columns } from "@/components/tables/billing/invoices";
import {
...
export const Route = createFileRoute("/dashboard/billing/invoices")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      status: z.enum(BillingInvoiceStatus).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableBillingInvoiceQuery, {
    ...
    return result.data?.billing?.billingInvoices || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(BillingInvoiceStatus).map((item) => ({
            label: item,
            value: item,
          }))}
