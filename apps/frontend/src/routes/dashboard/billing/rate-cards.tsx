import { columns } from "@/components/tables/billing/rate_cards";
import {
  ServiceType,
  execute,
  TableRateCardQuery,
...
export const Route = createFileRoute("/dashboard/billing/rate-cards")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableRateCardQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
      serviceType: context.search.serviceType,
    });

    return result.data?.billing?.rateCards || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, serviceType: v as any }),
            })
          }
          options={Object.values(ServiceType).map((item) => ({
            label: item,
            value: item,
          }))}
