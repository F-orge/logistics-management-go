import { columns } from "@/components/tables/billing/quotes";
import {
  QuoteStatus,
  execute,
  TableQuoteQuery,
...
export const Route = createFileRoute("/dashboard/billing/quotes")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableQuoteQuery, {
    ...
    return result.data?.billing?.quotes || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(QuoteStatus).map((item) => ({
            label: item,
            value: item,
          }))}
