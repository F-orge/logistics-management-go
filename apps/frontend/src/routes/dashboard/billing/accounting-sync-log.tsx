import { columns } from "@/components/tables/billing/accounting_sync_log";
import {
  SyncStatus,
  execute,
...
export const Route = createFileRoute("/dashboard/billing/accounting-sync-log")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      status: z.enum(SyncStatus).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", AccountingSyncLogsQuery, {
    ...
    return result.data?.billing?.accountingSyncLogs || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(SyncStatus).map((item) => ({
            label: item,
            value: item,
          }))}
