import { columns } from "@/components/tables/billing/client_accounts";
import { execute, TableClientAccountQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/billing/client-accounts")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableClientAccountQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.billing?.clientAccounts || [];
  },
});
