import { columns } from "@/components/tables/billing/credit_notes";
import { execute, TableCreditNoteQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/billing/credit-notes")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableCreditNoteQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.billing?.creditNotes || [];
  },
});
