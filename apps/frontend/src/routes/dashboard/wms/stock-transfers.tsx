...
export const Route = createFileRoute("/dashboard/wms/stock-transfers")({
  component: RouteComponent,
  validateSearch: zodValidator(
    z.object({
      page: z.number().min(1).default(1).catch(1),
      perPage: z.number().min(10).default(10).catch(10),
      status: z.enum(StockTransferStatus).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableStockTransferQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      status: context.search.status,
    });

    return result.data?.wms?.stockTransfers || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        <EnumSelect
          ...
          options={Object.values(StockTransferStatus).map((item) => ({
            label: item,
            value: item,
          }))}
