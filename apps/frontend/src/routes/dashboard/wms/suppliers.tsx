import { columns } from "@/components/tables/wms/suppliers";
import { execute, TableSupplierQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/wms/suppliers")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableSupplierQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.wms?.suppliers || [];
  },
});
