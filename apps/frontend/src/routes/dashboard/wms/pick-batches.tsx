import { columns } from "@/components/tables/wms/pick_batches";
import {
  PickBatchStatus,
  PickStrategy,
  execute,
...
export const Route = createFileRoute("/dashboard/wms/pick-batches")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      status: z.enum(PickBatchStatus).optional(),
      strategy: z.enum(PickStrategy).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TablePickBatchQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
      status: context.search.status,
      strategy: context.search.strategy,
    });

    return result.data?.wms?.pickBatches || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(PickBatchStatus).map((item) => ({
            label: item,
            value: item,
          }))}
        ...
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, strategy: v as any }),
            })
          }
          options={Object.values(PickStrategy).map((item) => ({
            label: item,
            value: item,
          }))}
