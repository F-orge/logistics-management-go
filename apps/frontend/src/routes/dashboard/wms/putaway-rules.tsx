import { columns } from "@/components/tables/wms/putaway_rules";
import {
  LocationType,
  execute,
...
export const Route = createFileRoute("/dashboard/wms/putaway-rules")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TablePutawayRuleQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      locationType: context.search.locationType,
    });

    return result.data?.wms?.putawayRules || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, locationType: v as any }),
            })
          }
          options={Object.values(LocationType).map((item) => ({
            label: item,
            value: item,
          }))}
