import { columns } from "@/components/tables/wms/packages";
import { execute, TablePackageQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/wms/packages")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TablePackageQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.wms?.packages || [];
  },
});
