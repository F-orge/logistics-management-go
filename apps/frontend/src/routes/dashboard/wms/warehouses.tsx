import { columns } from "@/components/tables/wms/warehouses";
import { execute, TableWarehouseQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/wms/warehouses")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableWarehouseQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.wms?.warehouses || [];
  },
});
