import { columns } from "@/components/tables/wms/tasks";
import {
  TaskStatus,
  TaskType,
  execute,
...
export const Route = createFileRoute("/dashboard/wms/tasks")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableTaskQuery, {
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(TaskType).map((item) => ({
            label: item,
            value: item,
          }))}
