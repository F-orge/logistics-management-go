import { columns } from "@/components/tables/wms/returns";
import {
  ReturnStatus,
  execute,
  TableReturnQuery,
...
export const Route = createFileRoute("/dashboard/wms/returns")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableReturnQuery, {
    ...
    return result.data?.wms?.returns || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(ReturnStatus).map((item) => ({
            label: item,
            value: item,
          }))}
