import { columns } from "@/components/tables/wms/inventory_adjustments";
import {
...
export const Route = createFileRoute("/dashboard/wms/inventory-adjustments")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      reason: z.enum(InventoryAdjustmentReason).optional(),
    })
  ),
  ...
  loader: async ({ context }) => {
    const result = await execute(
      ...
      TableInventoryAdjustmentQuery,
      {
        ...
        reason: context.search.reason,
      }
    );

    return result.data?.wms?.inventoryAdjustments || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          onValueChange={(v) =>
            navigate({
              search: (prev) => ({ ...prev, reason: v as any }),
            })
          }
          options={Object.values(InventoryAdjustmentReason).map((item) => ({
            label: item,
            value: item,
          }))}
