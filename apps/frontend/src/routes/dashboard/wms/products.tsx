import { columns } from "@/components/tables/wms/products";
import {
  ProductStatus,
  execute,
...
export const Route = createFileRoute("/dashboard/wms/products")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableWmsProductQuery, {
    ...
    return result.data?.wms?.wmsProducts || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(ProductStatus).map((item) => ({
            label: item,
            value: item,
          }))}
