import { columns } from "@/components/tables/wms/outbound_shipments";
import {
...
export const Route = createFileRoute("/dashboard/wms/outbound-shipments")({
  ...
  validateSearch: zodValidator(
    z.object({
      ...
      status: z.enum(OutboundShipmentStatus).optional(),
    })
  ),
  beforeLoad: ({ search }) => ({ search }),
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableOutboundShipmentQuery, {
    ...
    return result.data?.wms?.outboundShipments || [];
  },
});
    ...
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full flex flex-row items-center gap-2.5">
        ...
        <EnumSelect
          ...
          options={Object.values(OutboundShipmentStatus).map((item) => ({
            label: item,
            value: item,
          }))}
