import { columns } from "@/components/tables/wms/reorder_points";
import { execute, TableReorderPointQuery } from "@packages/graphql/client";
...
export const Route = createFileRoute("/dashboard/wms/reorder-points")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableReorderPointQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
    });

    return result.data?.wms?.reorderPoints || [];
  },
});
