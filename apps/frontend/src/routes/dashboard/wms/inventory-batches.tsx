import { columns } from "@/components/tables/wms/inventory_batches";
import { execute, TableInventoryBatchQuery } from "@packages/graphql/client";
import {
  DataTable,
  InputGroup,
  InputGroupAddon,
...
export const Route = createFileRoute("/dashboard/wms/inventory-batches")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableInventoryBatchQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
      search: context.search.search,
    });

    return result.data?.wms?.inventoryBatches || [];
  },
});
