import { columns } from "@/components/tables/wms/bin_thresholds";
import { execute, TableBinThresholdQuery } from "@packages/graphql/client";
...
export const Route = createFileRoute("/dashboard/wms/bin-thresholds")({
  ...
  loader: async ({ context }) => {
    const result = await execute("/api/graphql", TableBinThresholdQuery, {
      page: context.search.page,
      perPage: context.search.perPage,
    });

    return result.data?.wms?.binThresholds || [];
  },
});
