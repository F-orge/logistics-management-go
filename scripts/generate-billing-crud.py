#!/usr/bin/env python3
"""
Billing-Management CRUD Boilerplate Generator

This script generates complete CRUD implementations for all billing-management entities.
It converts monolithic .tsx files into a 4-file folder structure (create, update, delete, index).

Usage: python3 generate-billing-crud.py
"""
...
BM_PATH = os.path.join(BASE_PATH, "billing-management")

# All billing-management entities (kebab-case, CollectionEnum, PascalCaseSchema)
ENTITIES = [
    ("account-transactions", "BillingManagementAccountTransactions", "AccountTransactions"),
    ("client-accounts", "BillingManagementClientAccounts", "ClientAccounts"),
    ("credit-notes", "BillingManagementCreditNotes", "CreditNotes"),
    ("disputes", "BillingManagementDisputes", "Disputes"),
    ("invoice-line-items", "BillingManagementInvoiceLineItems", "InvoiceLineItems"),
    ("invoices", "BillingManagementInvoices", "Invoices"),
    ("logs", "BillingManagementLogs", "Logs"),
    ("payments", "BillingManagementPayments", "Payments"),
    ("quotes", "BillingManagementQuotes", "Quotes"),
    ("rate-cards", "BillingManagementRateCards", "RateCards"),
    ("rate-rules", "BillingManagementRateRules", "RateRules"),
    ("surcharges", "BillingManagementSurcharges", "Surcharges"),
]
...
import {{ {schema}Schema }} from "@/pocketbase/schemas/billing-management/{kebab}";
...
def main():
    """Generate all billing-management CRUD files."""
    print("💰 Billing-Management CRUD Boilerplate Generator")
    print("=" * 60)
    
    # Step 1: Create folders for each entity
    print("\n📁 Creating entity folders...")
    for kebab, _, _ in ENTITIES:
        entity_path = os.path.join(BM_PATH, kebab)
    ...
    for kebab, _, _ in ENTITIES:
        old_file = os.path.join(BM_PATH, f"{kebab}.tsx")
