...
async function seedPhase1Users() {
  ...
  console.log(
    `✓ Phase 1 Complete: Created ${Object.keys(ids.users).length} users`
  );
}
...
async function seedPhase3MarketingSales() {
  ...
  if (!ownerUserId || companyIds.length === 0 || crmProductIds.length === 0) {
    console.error(
      "✗ Missing prerequisite data. Phases 1 and 2 must run first."
    );
    return;
  }
  ...
  for (const campaignData of campaigns) {
    try {
      ...
      if (error instanceof ClientResponseError) {
        console.error(
          `    ✗ Failed to create ${campaignData.name}:`,
          error.message,
          error.response.data
        );
      } else {
  ...
  if (campaignIds.length === 0) {
    console.warn(
      "  ⚠ No campaigns created. Skipping leads and opportunities."
    );
    return;
  }
  ...
  for (const leadData of leads) {
    try {
      ...
      if (error instanceof ClientResponseError) {
        console.error(
          `    ✗ Failed to create ${leadData.name}:`,
          error.message,
          error.response.data
        );
      } else {
  ...
  for (const opportunityData of opportunities) {
    try {
      ...
      if (error instanceof ClientResponseError) {
        console.error(
          `    ✗ Failed to create ${opportunityData.name}:`,
          error.message,
          error.response.data
        );
      } else {
...
async function seedPhase4CRMSecondary() {
  ...
  for (const interactionData of interactions) {
    try {
      ...
      console.log(
        `    ✓ ${interaction.type} with contact (${interaction.outcome})`
      );
    } catch (error) {
      if (error instanceof ClientResponseError) {
        console.error(
  ...
  for (const caseData of cases) {
    try {
      const caseRecord = await pb
        .collection(Collections.CustomerRelationsCases)
        .create(caseData);
      console.log(
        `    ✓ ${caseRecord.caseNumber} (${caseRecord.priority} - ${caseRecord.status})`
      );
    } catch (error) {
      if (error instanceof ClientResponseError) {
        console.error(
  ...
  const invoices = Array.from({ length: 6 }, (_, i) => ({
    ...
    opportunity:
      opportunityIds.length > 0
        ? faker.helpers.arrayElement(opportunityIds)
        : undefined,
...
async function seedPhase6InventoryManagement() {
      ...
      const product = await pb
        ...
        .create({
          ...
          status: faker.helpers.arrayElement(
            Object.values(WarehouseManagementProductsStatusOptions)
          ),
          supplier:
  ...
  for (
    let i = 0;
    i < Math.min(5, Math.min(productIds.length, locationIds.length));
    i++
  ) {
    try {
      const stock = await pb
        ...
        .create({
          ...
          status: faker.helpers.arrayElement(
            Object.values(WarehouseManagementInventoryStockStatusOptions)
          ),
        });
    ...
    try {
      const vehicle = await pb
        ...
        .create({
          ...
          status: faker.helpers.arrayElement(
            Object.values(TransportManagementVehiclesStatusOptions)
          ),
        });
      ids.vehicles[`${vehicleTypes[i]}-${i}`] = vehicle.id;
      console.log(
        `    ✓ ${vehicleTypes[i]} (Registration: ${vehicle.registrationNumber})`
      );
    } catch (error) {
      if (error instanceof ClientResponseError) {
        console.error(
...
async function seedPhase8WarehouseOperations() {
  ...
  const salesOrderStatuses = Object.values(
    WarehouseManagementSalesOrdersStatusOptions
  );
  const inboundStatuses = Object.values(
    WarehouseManagementInboundShipmentsStatusOptions
  );
  const outboundStatuses = Object.values(
    WarehouseManagementOutboundShipmentsStatusOptions
  );
  ...
  const inboundShipments = Array.from({ length: 6 }, () => ({
    ...
    expectedArrivalDate: faker.date
      .future({ years: 1 })
      .toISOString()
      .split("T")[0],
  ...
  for (const shipmentData of inboundShipments) {
    try {
      const shipment = await pb
        ...
        .create(shipmentData);
      ids.inboundShipments[`INBOUND-${shipment.id.substring(0, 8)}`] =
        shipment.id;
      console.log(
        `    ✓ Inbound Shipment (${shipment.status}) - Expected: ${shipment.expectedArrivalDate}`
      );
        ...
        try {
          ...
          await pb
            ...
            .create({
              ...
              receivedQuantity:
                shipment.status === "received" ? expectedQty : undefined,
            });
        } catch (error) {
          if (error instanceof ClientResponseError) {
            console.error(
              `      ✗ Failed to add inbound shipment item:`,
              error.message,
              error.response.data
            );
          } else {
      ...
      if (error instanceof ClientResponseError) {
        console.error(
          `    ✗ Failed to create inbound shipment:`,
          error.message,
          error.response.data
        );
      } else {
    ...
    for (const shipmentData of outboundShipments) {
      try {
        const shipment = await pb
          ...
          .create(shipmentData);
        ids.outboundShipments[`OUTBOUND-${shipment.id.substring(0, 8)}`] =
          shipment.id;
        console.log(
          `    ✓ Outbound Shipment (${shipment.status}) - Tracking: ${shipment.trackingNumber}`
        );
        ...
        for (const productId of selectedProducts) {
          try {
            const selectedSalesOrderItem =
              faker.helpers.arrayElement(salesOrderItemIds);
            await pb
            ...
            if (error instanceof ClientResponseError) {
              console.error(
                `      ✗ Failed to add outbound shipment item:`,
                error.message,
                error.response.data
              );
            } else {
...
async function seedPhase9TransportOperations() {
  ...
  const tripStopStatuses = Object.values(
    TransportManagementTripStopsStatusOptions
  );
  ...
  for (const name of routeNames) {
    try {
      ...
      console.log(
        `    ✓ ${route.name} (Distance: ${route.totalDistance}km, Duration: ${route.totalDuration}h)`
      );
    } catch (error) {
      if (error instanceof ClientResponseError) {
        console.error(
          `    ✗ Failed to create route ${name}:`,
          error.message,
          error.response.data
        );
      } else {
  ...
  for (const tripData of trips) {
    try {
      ...
      console.log(
        `    ✓ Trip (${trip.status}) - Driver: ${tripData.driver}, Vehicle: ${tripData.vehicle}`
      );
      ...
      for (let seq = 1; seq <= numStops; seq++) {
        try {
          const stopData = {
            ...
            estimatedDepartureTime: faker.date
              .future({ years: 1 })
              .toISOString(),
            actualArrivalTime:
...
async function seedPhase10BillingSystem() {
  ...
  // 2. Seed Rate Rules
  const pricingModels = Object.values(
    BillingManagementRateRulesPricingModelOptions
  );
  ...
  // 3. Seed Surcharges
  const calculationMethods = Object.values(
    BillingManagementSurchargesCalculationMethodOptions
  );
  ...
  for (const companyId of companyIds) {
    try {
      ...
      (ids.clientAccounts as Record<string, string>)[companyId as string] =
        clientAccount.id;
    } catch (error) {
      if (error instanceof ClientResponseError) {
        console.error(
  ...
  console.log(
    `    ✓ Created ${Object.keys(ids.clientAccounts).length} client accounts`
  );
    ...
    try {
      const invoice = await pb
        ...
        .create({
          quote:
            quoteIds.length > 0
              ? faker.helpers.arrayElement(quoteIds)
              : undefined,
          status: faker.helpers.arrayElement(invoiceStatuses),
          issueDate: faker.date.past({ years: 1 }).toISOString().split("T")[0],
          dueDate: faker.date.future({ years: 1 }).toISOString().split("T")[0],
          ...
          notes: faker.commerce.productDescription(),
          sentAt: faker.datatype.boolean()
            ? faker.date.past({ years: 1 }).toISOString().split("T")[0]
            : undefined,
          paidAt: faker.datatype.boolean()
            ? faker.date.past({ years: 1 }).toISOString().split("T")[0]
            : undefined,
          createdBy: faker.helpers.arrayElement(userIds),
        });
  ...
  if (invoiceIds.length > 0 && disputeIds.length > 0) {
    for (let i = 0; i < 3; i++) {
      creditNoteData.push({
        ...
        issueDate: faker.date.past({ years: 1 }).toISOString().split("T")[0],
        appliedAt: faker.datatype.boolean()
          ? faker.date.past({ years: 1 }).toISOString().split("T")[0]
          : undefined,
        currency: "PHP",
  ...
  for (const noteData of creditNoteData) {
    try {
      const creditNote = await pb
        .collection(Collections.BillingManagementCreditNotes)
        .create(noteData);
      ids.creditNotes[`CREDIT-${creditNote.id.substring(0, 8)}`] =
        creditNote.id;
    } catch (error) {
      if (error instanceof ClientResponseError) {
        console.error(
  ...
  console.log(
    `    ✓ Created ${Object.keys(ids.creditNotes).length} credit notes`
  );
...
async function seedPhase11LogisticsExecution() {
  ...
  const shipmentStatuses = Object.values(
    TransportManagementShipmentLegsStatusOptions
  );
  ...
  console.log(
    `    ✓ Created ${Object.keys(ids.shipmentLegs).length} shipment legs`
  );
  ...
  for (const legId of shipmentLegIds) {
    ...
    for (let i = 0; i < numEvents; i++) {
      try {
        ...
        if (error instanceof ClientResponseError) {
          console.error(
            `      ✗ Failed to create shipment leg event:`,
            error.message,
            error.response.data
          );
        } else {
  ...
  for (const tripStopId of tripStopIds.slice(
    0,
    Math.min(5, tripStopIds.length)
  )) {
    try {
      await pb
...
async function seedPhase12DeliveryManagement() {
  ...
  const taskEventStatuses = Object.values(
    DeliveryManagementTaskEventsStatusOptions
  );
    ...
    try {
      const route = await pb
        ...
        .create({
          ...
          startedAt: faker.datatype.boolean()
            ? faker.date.past({ years: 1 }).toISOString()
            : undefined,
      ...
      if (error instanceof ClientResponseError) {
        console.error(
          `    ✗ Failed to create delivery route:`,
          error.message,
          error.response.data
        );
      } else {
  ...
  console.log(
    `    ✓ Created ${Object.keys(ids.deliveryRoutes).length} delivery routes`
  );
        ...
        .create({
          salesOrder:
            salesOrderIds.length > 0
              ? faker.helpers.arrayElement(salesOrderIds)
              : undefined,
          warehouse: faker.helpers.arrayElement(warehouseIds),
          type: faker.helpers.arrayElement([
            "Box",
          ...
          shippedAt: faker.datatype.boolean()
            ? faker.date.past({ years: 1 }).toISOString().split("T")[0]
            : undefined,
    ...
    try {
      const task = await pb
        ...
        .create({
          package:
            packageIds.length > 0
              ? faker.helpers.arrayElement(packageIds)
              : undefined,
          ...
          estimatedArrivalTime: faker.date.future({ years: 1 }).toISOString(),
          actualArrivalTime: faker.datatype.boolean()
            ? faker.date.past({ years: 1 }).toISOString()
            : undefined,
          deliveryTime: faker.datatype.boolean()
            ? faker.date.past({ years: 1 }).toISOString()
            : undefined,
          ...
          failureReason: faker.datatype.boolean()
            ? faker.helpers.arrayElement(failureReasons)
            : undefined,
        });
  ...
  console.log(
    `    ✓ Created ${Object.keys(ids.deliveryTasks).length} delivery tasks`
  );
  ...
  for (const taskId of deliveryTaskIds.slice(
    0,
    Math.min(8, deliveryTaskIds.length)
  )) {
    try {
      await pb
  ...
  for (const driverId of driverIds) {
    for (let i = 0; i < faker.number.int({ min: 3, max: 5 }); i++) {
      try {
        ...
        if (error instanceof ClientResponseError) {
          console.error(
            `      ✗ Failed to create driver location:`,
            error.message,
            error.response.data
          );
        } else {
  ...
  console.log(
    `✓ Phase 12 Complete: Created delivery routes, tasks, events, PODs, and driver locations`
  );
}
...
async function seedPhase13AdvancedOperational() {
  ...
  const expenseStatuses = Object.values(
    TransportManagementExpensesStatusOptions
  );
  const geofenceEventTypes = Object.values(
    TransportManagementGeofenceEventsTypeOptions
  );
  const billingLogStatuses = Object.values(BillingManagementLogsStatusOptions);
  const stockTransferStatuses = Object.values(
    WarehouseManagementStockTransferStatusOptions
  );
  const returnStatuses = Object.values(WarehouseManagementReturnsStatusOptions);
  const returnItemConditions = Object.values(
    WarehouseManagementReturnItemsConditionOptions
  );
  ...
  for (let i = 0; i < 15; i++) {
    try {
      ...
      if (error instanceof ClientResponseError) {
        console.error(
          `      ✗ Failed to create geofence event:`,
          error.message,
          error.response.data
        );
      } else {
  ...
  console.log(
    `    ✓ Created ${Object.keys(ids.stockTransfers).length} stock transfers`
  );
          ...
          .create({
            ...
            product:
              productIds.length > 0
                ...
                : undefined,
            batch:
              inventoryBatchIds.length > 0
                ? faker.helpers.arrayElement(inventoryBatchIds)
                : undefined,
