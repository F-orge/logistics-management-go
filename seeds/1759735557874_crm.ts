...
export async function seed(db: Kysely<DB>): Promise<void> {
  ...
  const campaignData = Array.from({ length: 20 }, () => generateCrmCampaign(faker))
  ...
  const productData = Array.from({ length: 50 }, () => generateCrmProduct(faker))
  ...
  const contactData = Array.from({ length: 200 }, () => {
    const companyId = Math.random() > 0.3 ? faker.helpers.arrayElement(companies).id : undefined
    return generateCrmContact(faker, faker.helpers.arrayElement(userIds), companyId)
  })
  ...
  const leadData = Array.from({ length: 150 }, () => {
    const campaignId = Math.random() > 0.4 ? faker.helpers.arrayElement(campaigns).id : undefined
    const convertedCompanyId =
      Math.random() > 0.8 ? faker.helpers.arrayElement(companies).id : undefined
  ...
  const opportunityData = Array.from({ length: 120 }, () => {
    const companyId = Math.random() > 0.2 ? faker.helpers.arrayElement(companies).id : undefined
    const contactId = Math.random() > 0.2 ? faker.helpers.arrayElement(contacts).id : undefined
    const campaignId = Math.random() > 0.5 ? faker.helpers.arrayElement(campaigns).id : undefined
  ...
  const caseData = Array.from({ length: 80 }, () => {
    const contactId = Math.random() > 0.3 ? faker.helpers.arrayElement(contacts).id : undefined
    return generateCrmCase(faker, faker.helpers.arrayElement(userIds), contactId)
  })
  ...
  const interactionData = Array.from({ length: 300 }, () => {
    ...
    const caseId = Math.random() > 0.7 ? faker.helpers.arrayElement(cases).id : undefined
    return generateCrmInteraction(faker, faker.helpers.arrayElement(userIds), contactId, caseId)
  })
  ...
  const invoiceData = Array.from({ length: 60 }, () => {
    const opportunityId =
      Math.random() > 0.5 ? faker.helpers.arrayElement(opportunities).id : undefined
  ...
  invoices.forEach((invoice) => {
    ...
    for (let i = 0; i < itemCount; i++) {
      invoiceItemData.push(
        generateCrmInvoiceItem(faker, invoice.id, faker.helpers.arrayElement(products).id),
      )
    }
  })
  ...
  const opportunityProductData: Array<ReturnType<typeof generateCrmOpportunityProduct>> = []
  ...
  companies.slice(0, 20).forEach((company) => {
    attachmentData.push(generateCrmAttachment(faker, company.id, CrmRecordType.Companies))
  })

  // Add attachments to contacts
  contacts.slice(0, 30).forEach((contact) => {
    attachmentData.push(generateCrmAttachment(faker, contact.id, CrmRecordType.Contacts))
  })

  // Add attachments to opportunities
  opportunities.slice(0, 25).forEach((opportunity) => {
    attachmentData.push(generateCrmAttachment(faker, opportunity.id, CrmRecordType.Opportunities))
  })
