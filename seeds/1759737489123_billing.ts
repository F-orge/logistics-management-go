...
export async function seed(db: Kysely<DB>): Promise<void> {
  ...
  const companies = await db.selectFrom('crm.companies').select(['id']).execute()
  ...
  const surchargesData = Array.from({ length: 15 }, () => generateBillingSurcharge(faker))
  ...
  const invoicesData = Array.from({ length: 80 }, () =>
    generateBillingInvoice(
      ...
      faker.helpers.arrayElement(companies).id,
      faker.helpers.arrayElement(users).id,
      faker.datatype.boolean() ? faker.helpers.arrayElement(quotes).id : undefined,
    ),
  )
  ...
  const disputesData = Array.from({ length: 20 }, () =>
    generateBillingDispute(
      ...
      faker.datatype.boolean() ? faker.helpers.arrayElement(users).id : undefined,
    ),
  )
  ...
  const creditNotesData = Array.from({ length: 15 }, () =>
    generateBillingCreditNote(
      faker,
      faker.helpers.arrayElement(invoices).id,
      faker.datatype.boolean() ? faker.helpers.arrayElement(disputes).id : undefined,
      faker.helpers.arrayElement(users).id,
    ),
  )
