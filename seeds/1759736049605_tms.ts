...
export async function seed(db: Kysely<DB>): Promise<void> {
  ...
  const carrierData = Array.from({ length: 25 }, () => generateTmsCarrier(faker))
  ...
  const vehicleData = Array.from({ length: 50 }, () => generateTmsVehicle(faker))
  ...
  const geofenceData = Array.from({ length: 20 }, () => generateTmsGeofence(faker))
  ...
  const driverScheduleData: Array<ReturnType<typeof generateTmsDriverSchedule>> = []
  ...
  const vehicleMaintenanceData: Array<ReturnType<typeof generateTmsVehicleMaintenance>> = []
  vehicles.forEach((vehicle) => {
    const maintenanceCount = faker.number.int({ min: 2, max: 5 })
    for (let i = 0; i < maintenanceCount; i++) {
      vehicleMaintenanceData.push(generateTmsVehicleMaintenance(faker, vehicle.id))
    }
  })
  const vehicleMaintenance = await vehicleMaintenanceRepo
    ...
    .onConflict((oc) => oc.doNothing())
    .execute()
  console.log(`✅ Created ${vehicleMaintenance.length} vehicle maintenance records`)

  // 5. Create trips
  console.log('🛣️ Creating trips...')
  const tripData = Array.from({ length: 80 }, () => {
    const driverId = Math.random() > 0.1 ? faker.helpers.arrayElement(drivers).id : undefined
    const vehicleId = Math.random() > 0.1 ? faker.helpers.arrayElement(vehicles).id : undefined
  ...
  vehicles.slice(0, 25).forEach((vehicle) => {
    ...
    for (let i = 0; i < eventCount; i++) {
      geofenceEventData.push(
        generateTmsGeofenceEvent(faker, faker.helpers.arrayElement(geofences).id, vehicle.id),
      )
    }
  })
  ...
  trips.forEach((trip) => {
    ...
    for (let i = 0; i < expenseCount; i++) {
      const driverId = Math.random() > 0.2 ? faker.helpers.arrayElement(drivers).id : undefined
  ...
  const shipmentLegData = Array.from({ length: 100 }, () => {
    ...
    const carrierId = Math.random() > 0.5 ? faker.helpers.arrayElement(carriers).id : undefined
    const internalTripId = Math.random() > 0.5 ? faker.helpers.arrayElement(trips).id : undefined
  ...
  const shipmentLegEventData: Array<ReturnType<typeof generateTmsShipmentLegEvent>> = []
  shipmentLegs.forEach((shipmentLeg) => {
    const eventCount = faker.number.int({ min: 2, max: 5 })
    for (let i = 0; i < eventCount; i++) {
      shipmentLegEventData.push(generateTmsShipmentLegEvent(faker, shipmentLeg.id))
    }
  })
  ...
  const partnerInvoiceItemData: Array<ReturnType<typeof generateTmsPartnerInvoiceItem>> = []
