...
export async function seed(db: Kysely<DB>): Promise<void> {
  ...
  const crmOpportunities = await db
    ...
    .execute();
  console.log(
    `📊 Found ${crmOpportunities.length} CRM opportunities for sales order linking`,
  );
  ...
  const crmCompanies = await db
    ...
    .execute();
  console.log(
    `📊 Found ${crmCompanies.length} CRM companies to use as clients`,
  );

  // 1. Create warehouses
  console.log('🏢 Creating warehouses...');
  const warehouseData = Array.from({ length: 8 }, () =>
    generateWmsWarehouse(faker),
  );
  ...
  const supplierData = Array.from({ length: 20 }, () =>
    generateWmsSupplier(faker),
  );
  ...
  locations
    .slice(0, Math.floor(locations.length * 0.6))
  ...
  const inventoryBatchData: Array<
    ReturnType<typeof generateWmsInventoryBatch>
  > = [];
  ...
  const inventoryStockData: Array<
    ReturnType<typeof generateWmsInventoryStock>
  > = [];
  products.forEach((product) => {
    ...
    for (let i = 0; i < stockLocationCount; i++) {
      ...
      const batchId =
        Math.random() > 0.4
          ? faker.helpers.arrayElement(inventoryBatches).id
          : undefined;
      inventoryStockData.push(
        generateWmsInventoryStock(faker, location.id, product.id, batchId),
      );
    }
  });
  ...
  const salesOrderItemData: Array<
    ReturnType<typeof generateWmsSalesOrderItem>
  > = [];
  ...
  const inboundShipmentItemData: Array<
    ReturnType<typeof generateWmsInboundShipmentItem>
  > = [];
  ...
  const inboundShipmentItems = await inboundShipmentItemRepo
    ...
    .execute();
  console.log(
    `✅ Created ${inboundShipmentItems.length} inbound shipment items`,
  );
  ...
  const outboundShipmentItemData: Array<
    ReturnType<typeof generateWmsOutboundShipmentItem>
  > = [];
  ...
  const outboundShipmentItems = await outboundShipmentItemRepo
    ...
    .execute();
  console.log(
    `✅ Created ${outboundShipmentItems.length} outbound shipment items`,
  );
  ...
  const packageData = salesOrders.slice(0, 80).map((salesOrder) => {
    const warehouseId = faker.helpers.arrayElement(warehouses).id;
    const packedByUserId =
      Math.random() > 0.2 ? faker.helpers.arrayElement(userIds) : undefined;
    return generateWmsPackage(
      faker,
      salesOrder.id,
      warehouseId,
      packedByUserId,
  ...
  const pickBatchData = Array.from({ length: 40 }, () => {
    ...
    const assignedUserId =
      Math.random() > 0.3 ? faker.helpers.arrayElement(userIds) : undefined;
    ...
    for (let i = 0; i < itemCount; i++) {
      pickBatchItemData.push(
        generateWmsPickBatchItem(
          faker,
          batch.id,
          faker.helpers.arrayElement(salesOrders).id,
        ),
      );
    }
  });
  ...
  const taskData = Array.from({ length: 120 }, () => {
    ...
    const pickBatchId =
      Math.random() > 0.6
        ? faker.helpers.arrayElement(pickBatches).id
        : undefined;
  ...
  tasks.forEach((task) => {
    ...
    for (let i = 0; i < itemCount; i++) {
      const sourceLocationId =
        Math.random() > 0.3
          ? faker.helpers.arrayElement(allLocations).id
          : undefined;
  ...
  const inventoryAdjustments = await inventoryAdjustmentRepo
    ...
    .execute();
  console.log(
    `✅ Created ${inventoryAdjustments.length} inventory adjustments`,
  );
