...
export async function seed(db: Kysely<DB>): Promise<void> {
  ...
  console.log(`Found ${drivers.length} drivers and ${packages.length} packages`)
  ...
  const customerTrackingLinks = await customerTrackingLinkRepo
    ...
    .onConflict((oc) => oc.doNothing())
    .execute()
  console.log(`✅ Created ${customerTrackingLinks.length} customer tracking links`)
  ...
  const completedTasks = deliveryTasks.filter(() => faker.datatype.boolean(0.3)) // 30% have proof
