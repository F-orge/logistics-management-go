...
func init() {
	m.Register(func(app core.App) error {
		...
		if err := json.Unmarshal([]byte(`{
			"updateRule": "(@request.auth.role = 'executive' || @request.auth.role = 'finance_dept') && (@request.body.id:isset = false || @request.body.invoiceNumber:isset = true || @request.body.customer:isset = false || @request.body.invoiceDate:isset = false || @request.body.orderRef:isset = false || @request.body.invoicePdf:isset = false)"
		}`), &collection); err != nil {
			return err
		}
