...
set
  ...
  quote_id = case when sqlc.arg(quote_id) is not null then
  ...
  amount_paid = case when sqlc.arg(amount_paid) is not null then
  ...
  tax_amount = case when sqlc.arg(tax_amount) is not null then
    sqlc.arg(tax_amount)::numeric
  else
    tax_amount
  end,
  discount_amount = case when sqlc.arg(discount_amount) is not null then
    sqlc.arg(discount_amount)::numeric
  else
    discount_amount
  end,
  subtotal = case when sqlc.arg(subtotal) is not null then
    sqlc.arg(subtotal)::numeric
  else
    subtotal
  end,
  payment_terms = case when sqlc.arg(payment_terms) is not null then
