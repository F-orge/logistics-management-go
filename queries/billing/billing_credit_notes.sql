...
set
  ...
  dispute_id = case when sqlc.arg(set_dispute_id)::boolean then
    sqlc.arg(dispute_id)::uuid
  else
    dispute_id
  end,
  credit_note_number = case when sqlc.arg(set_credit_note_number)::boolean then
  ...
  applied_at = case when sqlc.arg(set_applied_at)::boolean then
