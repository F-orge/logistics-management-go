...
set
  ...
  source_entity_id = case when sqlc.arg(set_source_entity_id)::boolean then
    sqlc.arg(source_entity_id)::uuid
  else
    source_entity_id
  end,
  source_entity_type = case when sqlc.arg(set_source_entity_type)::boolean then
  ...
  instructions = case when sqlc.arg(set_instructions)::boolean then
  ...
  start_time = case when sqlc.arg(set_start_time)::boolean then
    sqlc.arg(start_time)::timestamp
  else
    start_time
  end,
  end_time = case when sqlc.arg(set_end_time)::boolean then
