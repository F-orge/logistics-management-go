...
const tmsPaginateGeofenceMetadata = `-- name: TmsPaginateGeofenceMetadata :one
select
...
type TmsPaginateGeofenceMetadataParams struct {
	PerPage int32 `db:"per_page" json:"per_page"`
	Page    int32 `db:"page" json:"page"`
}

type TmsPaginateGeofenceMetadataRow struct {
...
func (q *Queries) TmsPaginateGeofenceMetadata(ctx context.Context, arg TmsPaginateGeofenceMetadataParams) (TmsPaginateGeofenceMetadataRow, error) {
	row := q.db.QueryRow(ctx, tmsPaginateGeofenceMetadata, arg.PerPage, arg.Page)
	var i TmsPaginateGeofenceMetadataRow
	err := row.Scan(
		&i.TotalItems,
		&i.TotalPages,
		&i.Page,
