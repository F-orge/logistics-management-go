...
async function seedUsers(): Promise<UsersRecord[]> {
  ...
  for (let i = 0; i < NUM_USERS; i++) {
    ...
    try {
      ...
      avatarFile = new File([blob], `avatar-${faker.string.uuid()}.png`, {
        type: blob.type,
      });
    } catch (err) {
...
async function seedDepartments(
  ...
  const potentialManagers = allUsers.filter(
    (u) =>
      u.role === UsersRoleOptions.manager || u.role === UsersRoleOptions.admin,
  );
  const potentialEmployees = allUsers.filter(
    (u) => u.role === UsersRoleOptions.employee,
  );
  for (let i = 0; i < NUM_DEPARTMENTS; i++) {
    ...
    try {
      ...
      avatarFile = new File([avatarBlob], `avatar-${faker.string.uuid()}.png`, {
        type: avatarBlob.type,
      });
      coverPhoto = new File([coverBlob], `cover-${faker.string.uuid()}.png`, {
        type: coverBlob.type,
      });
    } catch (err) {
      console.warn(`Failed to fetch avatar image, using URL instead.`, err);
        ...
        potentialEmployees.length > 0
          ? faker.helpers
              .arrayElements(
                ...
                faker.number.int({
                  min: 1,
                  max: Math.min(5, potentialEmployees.length),
                }),
              )
              .map((u) => u.id)
          : [],
    ...
    try {
      const record = await pb
        ...
        .create<DepartmentRecord>(departmentData);
...
async function seedEmployees(
  ...
  for (const department of allDepartments) {
    const numEmployees = faker.number.int({
      min: 1,
      max: Math.min(
        NUM_EMPLOYEES_PER_DEPARTMENT_MAX,
        usersWithoutEmployeeRecord.length,
      ),
    });
    ...
    for (let i = 0; i < numEmployees; i++) {
      ...
      const userIndex = faker.number.int({
        min: 0,
        max: usersWithoutEmployeeRecord.length - 1,
      });
      ...
      const employeeData: Partial<EmployeeRecord> = {
        ...
        last_name:
          user.name?.split(' ').slice(-1)[0] || faker.person.lastName(),
        middle_name: faker.datatype.boolean(0.3)
      ...
      try {
        const record = await pb
          ...
          .create<EmployeeRecord>(employeeData);
        createdEmployees.push(record);
        departmentEmployeeIds.push(record.id);
        console.log(
          `Created employee: ${record.first_name} ${record.last_name} (ID: ${record.id}) for user ${user.id}`,
        );
      } catch (error) {
    ...
    if (departmentEmployeeIds.length > 0) {
      try {
        ...
        console.log(
          `Updated department ${department.name} with ${departmentEmployeeIds.length} employees.`,
        );
      } catch (error) {
        console.error(
          `Failed to update department ${department.name} with employees:`,
          error,
        );
      }
    }
...
async function seedShipments(
  ...
  for (let i = 0; i < NUM_SHIPMENTS; i++) {
    const shipmentData: Partial<ShipmentRecord> = {
      ...
      price: Number.parseFloat(faker.commerce.price({ min: 100, max: 5000 })),
      ...
      return_attempts: faker.datatype.boolean(0.1)
    ...
    try {
      const record = await pb
        ...
        .create<ShipmentRecord>(shipmentData as ShipmentRecord);
      createdShipments.push(record);
      console.log(`Created shipment: ${record.id} for ${record.receiver_name}`);
    } catch (error) {
      console.error(
        `Failed to create shipment for ${shipmentData.receiver_name}:`,
        (error as any)?.response?.data || error,
      );
    }
  }
...
async function seedShipmentItems(
  ...
  for (const shipment of allShipments) {
    const numItems = faker.number.int({
      min: 1,
      max: NUM_SHIPMENT_ITEMS_PER_SHIPMENT_MAX,
    });
    for (let i = 0; i < numItems; i++) {
      ...
      try {
        const record = await pb
          ...
          .create<ShipmentItemRecord>(itemData);
        createdItems.push(record);
        console.log(
          `Created shipment item: ${record.name} for shipment ${shipment.id}`,
        );
      } catch (error) {
        console.error(
          `Failed to create shipment item ${itemData.name}:`,
          error,
        );
      }
    }
...
async function seedTasks(
  ...
  for (let i = 0; i < NUM_TASKS; i++) {
    ...
    const assignees = faker.helpers.arrayElements(
      ...
      faker.number.int({ min: 1, max: Math.min(3, allUsers.length - 1) }),
    );
    ...
    const taskData: Partial<TasksRecord> = {
      ...
      attachments: faker.datatype.boolean(0.2)
        ? Array.from({ length: faker.number.int({ min: 1, max: 2 }) }, () =>
            faker.system.commonFileName('zip'),
          )
        : [],
    };
    try {
...
async function seedChatMessages(
  ...
  for (let i = 0; i < NUM_USERS * 2; i++) {
    // Create a few messages per user on average
    ...
    try {
      const record = await pb
        .collection(Collections.ChatMessage)
        .create<ChatMessageRecord>(messageData);
      ...
      if (allShipments.length > 0 && faker.datatype.boolean(0.2)) {
        ...
        console.log(
          `Linked chat message ${record.id} to shipment ${shipment.id}`,
        );
      }
    } catch (error) {
...
async function seedTaskMessages(
  allTasks: TasksRecord[],
  allUsers: UsersRecord[],
): Promise<TasksMessagesRecord[]> {
  ...
  for (const task of allTasks) {
    const numMessages = faker.number.int({
      min: 0,
      max: NUM_MESSAGES_PER_ENTITY_MAX,
    });
    for (let i = 0; i < numMessages; i++) {
      ...
      try {
        const record = await pb
          .collection(Collections.TasksMessages)
          .create<TasksMessagesRecord>(messageData);
        createdTaskMessages.push(record);
        console.log(
          `Created task message for task ${task.id} by sender ${senderId}`,
        );
      } catch (error) {
        console.error(
...
async function seedNotifications(
  ...
  for (let i = 0; i < NUM_USERS * 1.5; i++) {
    // Average 1.5 notifications per user
    const user = faker.helpers.arrayElement(allUsers);
    const notificationData: Partial<NotificationsRecord> = {
      user: user.id,
      title: faker.company.catchPhrase(),
      message: faker.lorem.sentence(),
      link: faker.datatype.boolean(0.7)
        ? `/some/path/${faker.string.uuid()}`
        : undefined,
    };
    try {
      const record = await pb
        .collection(Collections.Notifications)
        .create<NotificationsRecord>(notificationData);
