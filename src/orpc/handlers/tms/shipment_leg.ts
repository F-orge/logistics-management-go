...
export const paginateShipmentLeg = implement(
  ...
  .handler(async ({ context, input }) => {
    const repo = new TmsShipmentLegRepository(context.db);
...
export const rangeShipmentLeg = implement(
  tmsContracts.rangeShipmentLegContract,
)
  .$context<HonoVariables>()
  .handler(async ({ context, input }) => {
    const repo = new TmsShipmentLegRepository(context.db);
...
export const inShipmentLeg = implement(
  tmsContracts.inShipmentLegContract,
)
  .$context<HonoVariables>()
  .handler(async ({ context, input }) => {
    const repo = new TmsShipmentLegRepository(context.db);
