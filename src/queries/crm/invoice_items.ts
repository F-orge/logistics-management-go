...
export const rangeInvoiceItem = (options: Parameters<typeof orpcClient.crm.rangeInvoiceItem>[0]) =>
  queryOptions({
    queryKey: ['crm.invoiceItems', options],
...
export const inInvoiceItem = (options: Parameters<typeof orpcClient.crm.inInvoiceItem>[0]) =>
  queryOptions({
    queryKey: ['crm.invoiceItems', options],
    queryFn: () => (options.length >= 1 ? orpcClient.crm.inInvoiceItem(options) : []),
    enabled: !!options,
  })
