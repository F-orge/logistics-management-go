...
export const paginateCampaign = (options: Parameters<typeof orpcClient.crm.paginateCampaign>[0]) =>
  queryOptions({
...
export const rangeCampaign = (options: Parameters<typeof orpcClient.crm.rangeCampaign>[0]) =>
  queryOptions({
    queryKey: ['crm.campaigns', options],
...
export const inCampaign = (options: Parameters<typeof orpcClient.crm.inCampaign>[0]) =>
  queryOptions({
    queryKey: ['crm.campaigns', options],
    queryFn: () => (options.length >= 1 ? orpcClient.crm.inCampaign(options) : []),
    enabled: !!options,
  })
