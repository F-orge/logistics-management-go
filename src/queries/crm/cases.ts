...
export const paginateCase = (options: Parameters<typeof orpcClient.crm.paginateCase>[0]) =>
  queryOptions({
    ...
    queryFn: async ({ client }) => {
      ...
      const owners = await client.ensureQueryData(inUser(cases.map((row) => row.ownerId)))
...
export const rangeCase = (options: Parameters<typeof orpcClient.crm.rangeCase>[0]) =>
  queryOptions({
    queryKey: ['crm.cases', options],
