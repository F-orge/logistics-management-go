...
export const paginateInteraction = (
  ...
  queryOptions({
    ...
    queryFn: async ({ client }) => {
      ...
      const userPromises = client.ensureQueryData(inUser(interactions.map((row) => row.userId)))
...
export const rangeInteraction = (options: Parameters<typeof orpcClient.crm.rangeInteraction>[0]) =>
  queryOptions({
    queryKey: ['crm.interactions', options],
...
export const inInteraction = (options: Parameters<typeof orpcClient.crm.inInteraction>[0]) =>
  queryOptions({
    queryKey: ['crm.interactions', options],
    queryFn: () => (options.length >= 1 ? orpcClient.crm.inInteraction(options) : []),
    enabled: !!options,
  })
