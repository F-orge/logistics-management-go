...
export const paginateCustomerTrackingLink = (
  ...
  queryOptions({
    ...
    queryFn: async ({ client }) => {
      const customerTrackingLinks = await orpcClient.dms.paginateCustomerTrackingLink(options)

      const deliveryTasks = await client.ensureQueryData(
        inDeliveryTask(customerTrackingLinks.map((row) => row.deliveryTaskId).filter(nonEmpty)),
      )
...
export const inCustomerTrackingLink = (
  ...
  queryOptions({
    ...
    queryFn: () => (options.length >= 1 ? orpcClient.dms.inCustomerTrackingLink(options) : []),
    enabled: !!options,
  })
