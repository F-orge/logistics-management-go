...
export const paginateTaskItem = (options: Parameters<typeof orpcClient.wms.paginateTaskItem>[0]) =>
  queryOptions({
    ...
    queryFn: async ({ client }) => {
      ...
      const locations = await client.ensureQueryData(
        inLocation(
          taskItems
            .flatMap((row) => [row.destinationLocationId, row.sourceLocationId])
            .filter(nonEmpty),
        ),
      )
...
export const rangeTaskItem = (options: Parameters<typeof orpcClient.wms.rangeTaskItem>[0]) =>
  queryOptions({
...
export const inTaskItem = (options: Parameters<typeof orpcClient.wms.inTaskItem>[0]) =>
  queryOptions({
