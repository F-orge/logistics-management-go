...
import { deleteQuote, paginateQuote, rangeQuote } from '@/queries/billing/quote'
...
export const Route = createFileRoute('/dashboard/billing/quote/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(billingQuoteSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateQuote(context.search)),
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteQuote, queryClient)
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'quoteNumber', // Assuming 'quoteNumber' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
