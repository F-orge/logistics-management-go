...
import { deleteInvoice, paginateInvoice, rangeInvoice } from '@/queries/billing/invoice'
...
export const Route = createFileRoute('/dashboard/billing/invoice/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(billingInvoiceSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateInvoice(context.search)),
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'invoiceNumber', // Assuming 'invoiceNumber' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
