...
import {
  ...
  rangeClientAccount,
} from '@/queries/billing/client_account'
...
export const Route = createFileRoute('/dashboard/billing/client-account/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(billingClientAccountSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateClientAccount(context.search)),
      chart: await context.queryClient.fetchQuery(rangeClientAccount({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteClientAccount, queryClient)
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'accountName', // Assuming 'accountName' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
