...
import { deleteSurcharge, paginateSurcharge, rangeSurcharge } from '@/queries/billing/surcharge'
...
export const Route = createFileRoute('/dashboard/billing/surcharge/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(billingSurchargeSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateSurcharge(context.search)),
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteSurcharge, queryClient)
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'name', // Assuming 'name' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
