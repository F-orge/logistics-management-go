...
import {
  deleteAccountingSyncLog,
  paginateAccountingSyncLog,
  rangeAccountingSyncLog,
} from '@/queries/billing/accounting_sync_log'
...
export const Route = createFileRoute('/dashboard/billing/accounting-sync-log/')({
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateAccountingSyncLog(context.search)),
      chart: await context.queryClient.fetchQuery(rangeAccountingSyncLog({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteAccountingSyncLog, queryClient)

  return (
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full">
        <h1 className="text-2xl font-bold">Accounting Sync Logs</h1>
      </section>
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'syncStatus', // Assuming 'syncStatus' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
      ...
      <section>
        <NewAccountingSyncLogFormDialog />
      </section>
    </article>
  )
}
