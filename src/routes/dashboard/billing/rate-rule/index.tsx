...
import { deleteRateRule, paginateRateRule, rangeRateRule } from '@/queries/billing/rate_rule'
...
export const Route = createFileRoute('/dashboard/billing/rate-rule/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(billingRateRuleSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateRateRule(context.search)),
      chart: await context.queryClient.fetchQuery(rangeRateRule({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteRateRule, queryClient)

  return (
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full">
        <h1 className="text-2xl font-bold">Rate Rules</h1>
      </section>
