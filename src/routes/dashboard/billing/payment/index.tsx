...
import { deletePayment, paginatePayment, rangePayment } from '@/queries/billing/payment'
...
export const Route = createFileRoute('/dashboard/billing/payment/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(billingPaymentSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginatePayment(context.search)),
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deletePayment, queryClient)
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'paymentMethod', // Assuming 'paymentMethod' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
