...
import {
  ...
  paginateCreditNote,
  rangeCreditNote,
} from '@/queries/billing/credit_note'
...
export const Route = createFileRoute('/dashboard/billing/credit-note/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(billingCreditNoteSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateCreditNote(context.search)),
      chart: await context.queryClient.fetchQuery(rangeCreditNote({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteCreditNote, queryClient)
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'creditNoteNumber', // Assuming 'creditNoteNumber' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
