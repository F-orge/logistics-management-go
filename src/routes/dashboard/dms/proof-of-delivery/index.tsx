...
} from '@/queries/dms/proof_of_delivery'
import { filterTransformer, paginateTransformer, sortTransformer } from '@/repositories/utils'
import { dmsProofOfDeliverySchema } from '@/schemas/dms/proof_of_delivery'
...
export const Route = createFileRoute('/dashboard/dms/proof-of-delivery/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(dmsProofOfDeliverySchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateProofOfDelivery(context.search)),
      chart: await context.queryClient.fetchQuery(rangeProofOfDelivery({ from, to })),
    }
  },
})
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'type', // Assuming 'type' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
