...
export const Route = createFileRoute('/dashboard/dms/customer-tracking-link/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(dmsCustomerTrackingLinkSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateCustomerTrackingLink(context.search)),
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteCustomerTrackingLink, queryClient)

  return (
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full">
        <h1 className="text-2xl font-bold">Customer Tracking Links</h1>
      </section>
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'trackingToken', // Assuming 'trackingToken' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
