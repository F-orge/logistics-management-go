...
import { deleteTaskEvent, paginateTaskEvent, rangeTaskEvent } from '@/queries/dms/task_event'
...
export const Route = createFileRoute('/dashboard/dms/task-event/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(dmsTaskEventSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateTaskEvent(context.search)),
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteTaskEvent, queryClient)

  return (
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full">
        <h1 className="text-2xl font-bold">Task Events</h1>
      </section>
