...
import {
  ...
  paginateDriverLocation,
  rangeDriverLocation,
} from '@/queries/dms/driver_location'
...
export const Route = createFileRoute('/dashboard/dms/driver-location/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(dmsDriverLocationSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateDriverLocation(context.search)),
      chart: await context.queryClient.fetchQuery(rangeDriverLocation({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteDriverLocation, queryClient)
            ...
            onClick={() =>
              navigate({
                search: (prev) => ({
                  ...
                  filters: [
                    {
                      column: 'latitude', // Assuming 'latitude' is a searchable field
                      operation: 'like',
                      value: `%${currentSearch}%`,
                    },
                  ],
