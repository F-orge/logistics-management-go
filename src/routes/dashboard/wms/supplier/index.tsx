...
import { deleteSupplier, paginateSupplier, rangeSupplier } from '@/queries/wms/supplier'
...
export const Route = createFileRoute('/dashboard/wms/supplier/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(wmsSupplierSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateSupplier(context.search)),
      chart: await context.queryClient.fetchQuery(rangeSupplier({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteSupplier, queryClient)
