...
import {
  ...
  paginateInventoryBatch,
  rangeInventoryBatch,
} from '@/queries/wms/inventory_batch'
...
export const Route = createFileRoute('/dashboard/wms/inventory-batch/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(wmsInventoryBatchSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateInventoryBatch(context.search)),
      chart: await context.queryClient.fetchQuery(rangeInventoryBatch({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteInventoryBatch, queryClient)
