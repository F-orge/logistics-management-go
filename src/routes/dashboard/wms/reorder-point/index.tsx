...
import {
  deleteReorderPoint,
  paginateReorderPoint,
  rangeReorderPoint,
} from '@/queries/wms/reorder_point'
...
export const Route = createFileRoute('/dashboard/wms/reorder-point/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(wmsReorderPointSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateReorderPoint(context.search)),
      chart: await context.queryClient.fetchQuery(rangeReorderPoint({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteReorderPoint, queryClient)

  return (
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full">
        <h1 className="text-2xl font-bold">Reorder Points</h1>
      </section>
