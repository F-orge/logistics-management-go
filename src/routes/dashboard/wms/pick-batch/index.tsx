...
import { deletePickBatch, paginatePickBatch, rangePickBatch } from '@/queries/wms/pick_batch'
...
export const Route = createFileRoute('/dashboard/wms/pick-batch/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(wmsPickBatchSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginatePickBatch(context.search)),
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deletePickBatch, queryClient)

  return (
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full">
        <h1 className="text-2xl font-bold">Pick Batches</h1>
      </section>
