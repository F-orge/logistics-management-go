...
import {
  ...
  paginateInboundShipment,
  rangeInboundShipment,
} from '@/queries/wms/inbound_shipment'
import { filterTransformer, paginateTransformer, sortTransformer } from '@/repositories/utils'
import { wmsInboundShipmentSchema } from '@/schemas/wms/inbound_shipment'
import NewInboundShipmentFormDialog from './-components/new'
import { columns } from './-components/table'

export const Route = createFileRoute('/dashboard/wms/inbound-shipment/')({
  component: RouteComponent,
  validateSearch: zodValidator(
    paginateTransformer().extend({
      filters: filterTransformer(wmsInboundShipmentSchema),
      sort: sortTransformer(wmsInboundShipmentSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateInboundShipment(context.search)),
      chart: await context.queryClient.fetchQuery(rangeInboundShipment({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteInboundShipment, queryClient)
