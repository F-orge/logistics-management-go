...
import { deleteTask, paginateTask, rangeTask } from '@/queries/wms/task'
...
export const Route = createFileRoute('/dashboard/wms/task/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(wmsTaskSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateTask(context.search)),
      chart: await context.queryClient.fetchQuery(rangeTask({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteTask, queryClient)
