...
import {
  deleteBinThreshold,
  paginateBinThreshold,
  rangeBinThreshold,
} from '@/queries/wms/bin_threshold'
...
export const Route = createFileRoute('/dashboard/wms/bin-threshold/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(wmsBinThresholdSchema).default([
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateBinThreshold(context.search)),
      chart: await context.queryClient.fetchQuery(rangeBinThreshold({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteBinThreshold, queryClient)

  return (
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full">
        <h1 className="text-2xl font-bold">Bin Thresholds</h1>
      </section>
