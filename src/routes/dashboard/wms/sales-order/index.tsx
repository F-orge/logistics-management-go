...
import { deleteSalesOrder, paginateSalesOrder, rangeSalesOrder } from '@/queries/wms/sales_order'
...
export const Route = createFileRoute('/dashboard/wms/sales-order/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(wmsSalesOrderSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateSalesOrder(context.search)),
      chart: await context.queryClient.fetchQuery(rangeSalesOrder({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteSalesOrder, queryClient)
