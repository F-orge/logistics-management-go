...
import { deleteExpense, paginateExpense, rangeExpense } from '@/queries/tms/expense'
...
export const Route = createFileRoute('/dashboard/tms/expense/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(tmsExpenseSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateExpense(context.search)),
      chart: await context.queryClient.fetchQuery(rangeExpense({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteExpense, queryClient)
