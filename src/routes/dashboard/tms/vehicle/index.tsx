...
import { deleteVehicle, paginateVehicle, rangeVehicle } from '@/queries/tms/vehicle'
...
export const Route = createFileRoute('/dashboard/tms/vehicle/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(tmsVehicleSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateVehicle(context.search)),
      chart: await context.queryClient.fetchQuery(rangeVehicle({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteVehicle, queryClient)
