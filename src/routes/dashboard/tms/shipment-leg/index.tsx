...
export const Route = createFileRoute('/dashboard/tms/shipment-leg/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(tmsShipmentLegSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateShipmentLeg(context.search)),
      chart: await context.queryClient.fetchQuery(rangeShipmentLeg({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteShipmentLeg, queryClient)

  return (
    <article className="grid grid-cols-12 gap-5">
      <section className="col-span-full">
        <h1 className="text-2xl font-bold">Shipment Legs</h1>
      </section>
