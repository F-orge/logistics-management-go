...
import { deleteRoute, paginateRoute, rangeRoute } from '@/queries/tms/route'
...
export const Route = createFileRoute('/dashboard/tms/route/')({
  ...
  validateSearch: zodValidator(
    paginateTransformer().extend({
      ...
      sort: sortTransformer(tmsRouteSchema).default([{ column: 'createdAt', order: 'desc' }]),
  ...
  async loader({ context }) {
    ...
    return {
      dataTable: await context.queryClient.fetchQuery(paginateRoute(context.search)),
      chart: await context.queryClient.fetchQuery(rangeRoute({ from, to })),
    }
  },
})
...
function RouteComponent() {
  ...
  const deleteMutation = useMutation(deleteRoute, queryClient)
