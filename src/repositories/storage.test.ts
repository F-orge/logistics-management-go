...
describe('BunStorageRepository', () => {
  ...
  it.each(getCases)('get: $name', async ({ setup }) => {
    const { recordId, recordType, name, expected, expectError } =
      await setup(repo);
    let error = null;
  ...
  it.each(deleteCases)('delete: $name', async ({ setup }) => {
    const { recordId, recordType, fileName, expected, expectError } =
      await setup(repo);
    let error = null;
    let result = false;
  ...
  it.each(statCases)('stat: $name', async ({ setup }) => {
    ...
    const { recordId, recordType, fileName, expectedSize, expectError } =
      result;
    const expectedErrorType =
      'expectedErrorType' in result ? result.expectedErrorType : undefined;
    ...
    if (expectError) {
      ...
      if (statResult) {
        ...
        expect(statResult.mtime.getTime()).toBeGreaterThan(
          oneMinuteAgo.getTime(),
        );
        expect(statResult.ctime.getTime()).toBeGreaterThan(
          oneMinuteAgo.getTime(),
        );
        expect(statResult.atime.getTime()).toBeGreaterThan(
          oneMinuteAgo.getTime(),
        );
      }
    }
