...
import type { BillingTransactionTypeEnum, DB } from '@/db/types'
  ...
  implements GenericRepository<'billing.accountTransactions'>
{
  ...
  > {
    return this.db.selectFrom('billing.accountTransactions').selectAll().where('id', 'in', values)
  }
  create(
    value: {
  ...
  delete(id: string): DeleteQueryBuilder<DB, 'billing.accountTransactions', DeleteResult> {
...
export class BillingAccountTransactionRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('billing.accountTransactions.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['billing.accountTransactions']>[]) {
    return this.db.insertInto('billing.accountTransactions').values(values).returningAll()
  }
