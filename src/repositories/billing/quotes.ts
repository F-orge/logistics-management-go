...
import type { BillingQuoteStatusEnum, DB } from '@/db/types'
...
export class QuoteRepository implements GenericRepository<'billing.quotes'> {
  ...
  > {
    return this.db.selectFrom('billing.quotes').selectAll().where('id', 'in', values)
  }
  create(
    value: {
      destinationDetails: string
  ...
  > {
    return this.db.updateTable('billing.quotes').set(value).where('id', '=', id).returningAll()
  }
...
export class BillingQuoteRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('billing.quotes.id', 'like', `%${search}%`)
  ...
  update(id: DB['billing.quotes']['id']['__update__'], value: Updateable<DB['billing.quotes']>) {
    return this.db
  ...
  delete(id: DB['billing.quotes']['id']['__update__']) {
    return this.db.deleteFrom('billing.quotes').where('billing.quotes.id', '=', id)
  }
}
