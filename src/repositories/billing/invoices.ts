...
import type { BillingInvoiceStatusEnum, DB } from '@/db/types'
import type { FilterConfig, GenericRepository, SortConfig } from '../interface'

export class InvoiceRepository implements GenericRepository<'billing.invoices'> {
  ...
  > {
    return this.db.selectFrom('billing.invoices').selectAll().where('id', 'in', values)
  }
  create(
    value: {
      clientId: string
  ...
  > {
    return this.db.updateTable('billing.invoices').set(value).where('id', '=', id).returningAll()
  }
...
export class BillingInvoiceRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('billing.invoices.id', 'like', `%${search}%`)
  ...
  delete(id: DB['billing.invoices']['id']['__update__']) {
    return this.db.deleteFrom('billing.invoices').where('billing.invoices.id', '=', id)
  }
}
