...
export class InvoiceLineItemRepository implements GenericRepository<'billing.invoiceLineItems'> {
  ...
  > {
    return this.db.selectFrom('billing.invoiceLineItems').selectAll().where('id', 'in', values)
  }
  create(
    value: {
      description: string
  ...
  delete(id: string): DeleteQueryBuilder<DB, 'billing.invoiceLineItems', DeleteResult> {
...
export class BillingInvoiceLineItemRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('billing.invoiceLineItems.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['billing.invoiceLineItems']>[]) {
    return this.db.insertInto('billing.invoiceLineItems').values(values).returningAll()
  }
