...
import type { BillingSurchargeCalculationMethodEnum, DB } from '@/db/types'
import type { FilterConfig, GenericRepository, SortConfig } from '../interface'

export class SurchargeRepository implements GenericRepository<'billing.surcharges'> {
  ...
  > {
    return this.db.selectFrom('billing.surcharges').selectAll().where('id', 'in', values)
  }
  create(
    value: {
  ...
  > {
    return this.db.updateTable('billing.surcharges').set(value).where('id', '=', id).returningAll()
  }
  delete(id: string): DeleteQueryBuilder<DB, 'billing.surcharges', DeleteResult> {
...
export class BillingSurchargeRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('billing.surcharges.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['billing.surcharges']>[]) {
    return this.db.insertInto('billing.surcharges').values(values).returningAll()
  }
  ...
  delete(id: DB['billing.surcharges']['id']['__update__']) {
    return this.db.deleteFrom('billing.surcharges').where('billing.surcharges.id', '=', id)
  }
}
