...
export class LocationRepository implements GenericRepository<'wms.locations'> {
  ...
  > {
    return this.db.selectFrom('wms.locations').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('wms.locations').set(value).where('id', '=', id).returningAll()
  }
...
export class WmsLocationRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.locations.id', 'like', `%${search}%`)
  ...
  update(id: DB['wms.locations']['id']['__update__'], value: Updateable<DB['wms.locations']>) {
    return this.db
  ...
  delete(id: DB['wms.locations']['id']['__update__']) {
    return this.db.deleteFrom('wms.locations').where('wms.locations.id', '=', id)
  }
}
