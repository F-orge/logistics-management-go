...
import type { DB, WmsProductStatusEnum } from '@/db/types'
...
export class ProductRepository implements GenericRepository<'wms.products'> {
  ...
  > {
    return this.db.selectFrom('wms.products').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('wms.products').set(value).where('id', '=', id).returningAll()
  }
...
export class WmsProductRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.products.id', 'like', `%${search}%`)
  ...
  update(id: DB['wms.products']['id']['__update__'], value: Updateable<DB['wms.products']>) {
    return this.db
