...
import type { DB, WmsSalesOrderStatusEnum } from '@/db/types'
import type { FilterConfig, GenericRepository, SortConfig } from '../interface'

export class SalesOrderRepository implements GenericRepository<'wms.salesOrders'> {
  ...
  > {
    return this.db.selectFrom('wms.salesOrders').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('wms.salesOrders').set(value).where('id', '=', id).returningAll()
  }
...
export class WmsSalesOrderRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.salesOrders.id', 'like', `%${search}%`)
  ...
  update(id: DB['wms.salesOrders']['id']['__update__'], value: Updateable<DB['wms.salesOrders']>) {
    return this.db
  ...
  delete(id: DB['wms.salesOrders']['id']['__update__']) {
    return this.db.deleteFrom('wms.salesOrders').where('wms.salesOrders.id', '=', id)
  }
}
