...
import type { DB, WmsStockTransferStatusEnum } from '@/db/types'
import type { FilterConfig, GenericRepository, SortConfig } from '../interface'

export class StockTransferRepository implements GenericRepository<'wms.stockTransfers'> {
  ...
  > {
    return this.db.selectFrom('wms.stockTransfers').selectAll().where('id', 'in', values)
  }
  create(
    value: {
      destinationWarehouseId: string
  ...
  > {
    return this.db.updateTable('wms.stockTransfers').set(value).where('id', '=', id).returningAll()
  }
  delete(id: string): DeleteQueryBuilder<DB, 'wms.stockTransfers', DeleteResult> {
...
export class WmsStockTransferRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.stockTransfers.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['wms.stockTransfers']>[]) {
    return this.db.insertInto('wms.stockTransfers').values(values).returningAll()
  }
  ...
  delete(id: DB['wms.stockTransfers']['id']['__update__']) {
    return this.db.deleteFrom('wms.stockTransfers').where('wms.stockTransfers.id', '=', id)
  }
}
