...
export class BinThresholdRepository implements GenericRepository<'wms.binThresholds'> {
  ...
  > {
    return this.db.selectFrom('wms.binThresholds').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('wms.binThresholds').set(value).where('id', '=', id).returningAll()
  }
  delete(id: string): DeleteQueryBuilder<DB, 'wms.binThresholds', DeleteResult> {
...
export class WmsBinThresholdRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.binThresholds.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['wms.binThresholds']>[]) {
    return this.db.insertInto('wms.binThresholds').values(values).returningAll()
  }
  ...
  delete(id: DB['wms.binThresholds']['id']['__update__']) {
    return this.db.deleteFrom('wms.binThresholds').where('wms.binThresholds.id', '=', id)
  }
}
