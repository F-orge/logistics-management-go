...
export class ReorderPointRepository implements GenericRepository<'wms.reorderPoints'> {
  ...
  > {
    return this.db.selectFrom('wms.reorderPoints').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('wms.reorderPoints').set(value).where('id', '=', id).returningAll()
  }
  delete(id: string): DeleteQueryBuilder<DB, 'wms.reorderPoints', DeleteResult> {
...
export class WmsReorderPointRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.reorderPoints.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['wms.reorderPoints']>[]) {
    return this.db.insertInto('wms.reorderPoints').values(values).returningAll()
  }
  ...
  delete(id: DB['wms.reorderPoints']['id']['__update__']) {
    return this.db.deleteFrom('wms.reorderPoints').where('wms.reorderPoints.id', '=', id)
  }
}
