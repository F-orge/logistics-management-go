...
import type { DB, WmsTaskItemStatusEnum } from '@/db/types'
...
export class TaskItemRepository implements GenericRepository<'wms.taskItems'> {
  ...
  > {
    return this.db.selectFrom('wms.taskItems').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('wms.taskItems').set(value).where('id', '=', id).returningAll()
  }
...
export class WmsTaskItemRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.taskItems.id', 'like', `%${search}%`)
  ...
  update(id: DB['wms.taskItems']['id']['__update__'], value: Updateable<DB['wms.taskItems']>) {
    return this.db
  ...
  delete(id: DB['wms.taskItems']['id']['__update__']) {
    return this.db.deleteFrom('wms.taskItems').where('wms.taskItems.id', '=', id)
  }
}
