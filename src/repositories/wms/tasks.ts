...
import type { DB, WmsTaskStatusEnum, WmsTaskTypeEnum } from '@/db/types'
...
export class TaskRepository implements GenericRepository<'wms.tasks'> {
  ...
  > {
    return this.db.selectFrom('wms.tasks').selectAll().where('id', 'in', values)
  }
  create(
    value: {
      taskNumber: string
  ...
  > {
    return this.db.updateTable('wms.tasks').set(value).where('id', '=', id).returningAll()
  }
...
export class WmsTaskRepository {
  ...
  update(id: DB['wms.tasks']['id']['__update__'], value: Updateable<DB['wms.tasks']>) {
    return this.db.updateTable('wms.tasks').set(value).where('wms.tasks.id', '=', id).returningAll()
  }
