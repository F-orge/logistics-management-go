...
export class PackageRepository implements GenericRepository<'wms.packages'> {
  ...
  > {
    return this.db.selectFrom('wms.packages').selectAll().where('id', 'in', values)
  }
  create(
    value: {
      packageNumber: string
  ...
  > {
    return this.db.updateTable('wms.packages').set(value).where('id', '=', id).returningAll()
  }
...
export class WmsPackageRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.packages.id', 'like', `%${search}%`)
  ...
  update(id: DB['wms.packages']['id']['__update__'], value: Updateable<DB['wms.packages']>) {
    return this.db
