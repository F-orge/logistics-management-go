...
export class SupplierRepository implements GenericRepository<'wms.suppliers'> {
  ...
  > {
    return this.db.selectFrom('wms.suppliers').selectAll().where('id', 'in', values)
  }
  create(
    value: { name: string } & {
  ...
  > {
    return this.db.updateTable('wms.suppliers').set(value).where('id', '=', id).returningAll()
  }
...
export class WmsSupplierRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('wms.suppliers.id', 'like', `%${search}%`)
  ...
  update(id: DB['wms.suppliers']['id']['__update__'], value: Updateable<DB['wms.suppliers']>) {
    return this.db
  ...
  delete(id: DB['wms.suppliers']['id']['__update__']) {
    return this.db.deleteFrom('wms.suppliers').where('wms.suppliers.id', '=', id)
  }
}
