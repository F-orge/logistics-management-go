...
import { CrmCompany, type DB } from '@/db/types'
...
export class CompanyRepository implements GenericRepository<'crm.companies'> {
  ...
  > {
    return this.db.updateTable('crm.companies').set(value).where('id', '=', id).returningAll()
  }
  ...
  > {
    return this.db.selectFrom('crm.companies').selectAll().where('id', 'in', values)
  }
...
export class CrmCompanyRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('crm.companies.name', 'like', `%${search}%`)
  ...
  update(id: DB['crm.companies']['id']['__update__'], value: Updateable<DB['crm.companies']>) {
    return this.db
  ...
  delete(id: DB['crm.companies']['id']['__update__']) {
    return this.db.deleteFrom('crm.companies').where('crm.companies.id', '=', id)
  }
}
