...
export class CampaignRepository implements GenericRepository<'crm.campaigns'> {
  ...
  > {
    return this.db.selectFrom('crm.campaigns').selectAll().where('id', 'in', values)
  }
  create(
    value: { name: string } & {
  ...
  > {
    return this.db.updateTable('crm.campaigns').set(value).where('id', '=', id).returningAll()
  }
...
export class CrmCampaignRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('crm.campaigns.name', 'like', `%${search}%`)
  ...
  update(id: DB['crm.campaigns']['id']['__update__'], value: Updateable<DB['crm.campaigns']>) {
    return this.db
  ...
  delete(id: DB['crm.campaigns']['id']['__update__']) {
    return this.db.deleteFrom('crm.campaigns').where('crm.campaigns.id', '=', id)
  }
}
