...
export class ContactRepository implements GenericRepository<'crm.contacts'> {
  ...
  > {
    return this.db.selectFrom('crm.contacts').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('crm.contacts').set(value).where('id', '=', id).returningAll()
  }
...
export class CrmContactRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('crm.contacts.name', 'like', `%${search}%`)
  ...
  update(id: DB['crm.contacts']['id']['__update__'], value: Updateable<DB['crm.contacts']>) {
    return this.db
