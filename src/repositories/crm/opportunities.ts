...
import type { CrmOpportunitySource, CrmOpportunityStage, DB } from '@/db/types'
import type { FilterConfig, GenericRepository, SortConfig } from '../interface'

export class OpportunityRepository implements GenericRepository<'crm.opportunities'> {
  ...
  > {
    return this.db.selectFrom('crm.opportunities').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('crm.opportunities').set(value).where('id', '=', id).returningAll()
  }
  delete(id: string): DeleteQueryBuilder<DB, 'crm.opportunities', DeleteResult> {
...
export class CrmOpportunityRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('crm.opportunities.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['crm.opportunities']>[]) {
    return this.db.insertInto('crm.opportunities').values(values).returningAll()
  }
  ...
  delete(id: DB['crm.opportunities']['id']['__update__']) {
    return this.db.deleteFrom('crm.opportunities').where('crm.opportunities.id', '=', id)
  }
}
