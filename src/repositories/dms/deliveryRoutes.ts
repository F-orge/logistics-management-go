...
import type { DB, DmsDeliveryRouteStatusEnum } from '@/db/types'
import type { FilterConfig, GenericRepository, SortConfig } from '../interface'

export class DeliveryRouteRepository implements GenericRepository<'dms.deliveryRoutes'> {
  ...
  > {
    return this.db.selectFrom('dms.deliveryRoutes').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.insertInto('dms.deliveryRoutes').values(value).returningAll()
  }
  update(
    id: string,
    value: {
  ...
  > {
    return this.db.updateTable('dms.deliveryRoutes').set(value).where('id', '=', id).returningAll()
  }
  delete(id: string): DeleteQueryBuilder<DB, 'dms.deliveryRoutes', DeleteResult> {
...
export class DmsDeliveryRouteRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('dms.deliveryRoutes.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['dms.deliveryRoutes']>[]) {
    return this.db.insertInto('dms.deliveryRoutes').values(values).returningAll()
  }
  ...
  delete(id: DB['dms.deliveryRoutes']['id']['__update__']) {
    return this.db.deleteFrom('dms.deliveryRoutes').where('dms.deliveryRoutes.id', '=', id)
  }
}
