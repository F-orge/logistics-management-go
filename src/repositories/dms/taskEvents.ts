...
import type { DB, DmsTaskEventStatusEnum } from '@/db/types'
...
export class TaskEventRepository implements GenericRepository<'dms.taskEvents'> {
  ...
  > {
    return this.db.selectFrom('dms.taskEvents').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('dms.taskEvents').set(value).where('id', '=', id).returningAll()
  }
...
export class DmsTaskEventRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('dms.taskEvents.id', 'like', `%${search}%`)
  ...
  update(id: DB['dms.taskEvents']['id']['__update__'], value: Updateable<DB['dms.taskEvents']>) {
    return this.db
  ...
  delete(id: DB['dms.taskEvents']['id']['__update__']) {
    return this.db.deleteFrom('dms.taskEvents').where('dms.taskEvents.id', '=', id)
  }
}
