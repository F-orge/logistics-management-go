...
export class GeofenceRepository implements GenericRepository<'tms.geofences'> {
  ...
  > {
    return this.db.selectFrom('tms.geofences').selectAll().where('id', 'in', values)
  }
  create(
    value: { name: string } & {
  ...
  > {
    return this.db.updateTable('tms.geofences').set(value).where('id', '=', id).returningAll()
  }
...
export class TmsGeofenceRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('tms.geofences.id', 'like', `%${search}%`)
  ...
  update(id: DB['tms.geofences']['id']['__update__'], value: Updateable<DB['tms.geofences']>) {
    return this.db
  ...
  delete(id: DB['tms.geofences']['id']['__update__']) {
    return this.db.deleteFrom('tms.geofences').where('tms.geofences.id', '=', id)
  }
}
