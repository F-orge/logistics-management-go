...
import type { DB, TmsShipmentLegStatusEnum } from '@/db/types'
import type { FilterConfig, GenericRepository, SortConfig } from '../interface'

export class ShipmentLegRepository implements GenericRepository<'tms.shipmentLegs'> {
  ...
  > {
    return this.db.selectFrom('tms.shipmentLegs').selectAll().where('id', 'in', values)
  }
  create(
  ...
  > {
    return this.db.updateTable('tms.shipmentLegs').set(value).where('id', '=', id).returningAll()
  }
...
export class TmsShipmentLegRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('tms.shipmentLegs.id', 'like', `%${search}%`)
  ...
  delete(id: DB['tms.shipmentLegs']['id']['__update__']) {
    return this.db.deleteFrom('tms.shipmentLegs').where('tms.shipmentLegs.id', '=', id)
  }
}
