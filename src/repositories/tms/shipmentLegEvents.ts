...
export class ShipmentLegEventRepository implements GenericRepository<'tms.shipmentLegEvents'> {
  ...
  > {
    return this.db.selectFrom('tms.shipmentLegEvents').selectAll().where('id', 'in', values)
  }
  create(
  ...
  delete(id: string): DeleteQueryBuilder<DB, 'tms.shipmentLegEvents', DeleteResult> {
...
export class TmsShipmentLegEventRepository {
  ...
  select(
    ...
    if (search) builder = builder.where('tms.shipmentLegEvents.id', 'like', `%${search}%`)
  ...
  batchCreate(values: Insertable<DB['tms.shipmentLegEvents']>[]) {
    return this.db.insertInto('tms.shipmentLegEvents').values(values).returningAll()
  }
  ...
  delete(id: DB['tms.shipmentLegEvents']['id']['__update__']) {
    return this.db.deleteFrom('tms.shipmentLegEvents').where('tms.shipmentLegEvents.id', '=', id)
  }
}
