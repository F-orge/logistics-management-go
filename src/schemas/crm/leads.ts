...
export const crmLeadSchema = z.object({
  ...
  campaignId: z.uuid({ message: 'Invalid UUID format for campaign ID' }).nullable(),
  convertedAt: z.iso.datetime({ message: 'Invalid ISO datetime format for conversion date' }).nullable(),
  convertedCompanyId: z.uuid({ message: 'Invalid UUID format for converted company ID' }).nullable(),
  convertedContactId: z.uuid({ message: 'Invalid UUID format for converted contact ID' }).nullable(),
  convertedOpportunityId: z.uuid({ message: 'Invalid UUID format for converted opportunity ID' }).nullable(),
  leadScore: z
    ...
    .nullable(),
  leadSource: z.enum(CrmLeadSource, { message: 'Invalid lead source' }).nullable(),
