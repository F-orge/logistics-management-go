...
export type RelationFieldProps<Records extends RelationItem> = {
  ...
  required?: boolean;
  filterQuery?: (query: string, id?: string) => string;
};
    ...
    async (query?: string): Promise<Records[]> => {
      try {
        ...
        if (field.state.value) {
          filters.push(`id ?= "${field.state.value}"`);
        }

        if (query && displayField) {
          filters.push(`${displayField} ~ "${query}"`);
        }

        const filter = filters.length > 0 ? filters.join(" || ") : undefined;

        const finalFilter = props.filterQuery
          ? props.filterQuery(query || "", field.state.value)
          : filter;
