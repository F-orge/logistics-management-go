...
const CompanyControls = ({
  ...
  const handleSearch = () => {
    ...
    const filterQuery = `(name ~ '${searchTerm}' || street ~ '${searchTerm}' || city ~ '${searchTerm}' || state ~ '${searchTerm}' || postalCode ~ '${searchTerm}' || country ~ '${searchTerm}' || phoneNumber ~ '${searchTerm}' || industry ~ '${searchTerm}' || website ~ '${searchTerm}')`;
