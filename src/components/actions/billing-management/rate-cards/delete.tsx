...
const DeleteRateCardFormDialog = () => {
	...
	const handleDelete = async () => {
		if (!searchParams.id) {
			toast.error("No Ratecard selected");
			return;
		}
	...
	return (
		<AlertDialog
			open={searchParams.action === "deleteRateCard"}
			onOpenChange={(open) => {
				if (!open) {
					navigate({
						search: (prev) => ({
		...
		>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>Delete RateCard</AlertDialogTitle>
					<AlertDialogDescription>
...
export default DeleteRateCardFormDialog;
