...
export const CreateSuppliers = () => {
  ...
  return (
    <FormDialog
      ...
      description="Fill in the details to create a new suppliers."
...
export const UpdateSuppliers = () => {
  ...
  const { data: record } = useQuery({
    queryKey: [Collections.WarehouseManagementSuppliers, searchQuery.id],
    queryFn: async () =>
      pocketbase
        .collection(Collections.WarehouseManagementSuppliers)
  ...
  return (
    <FormDialog
      ...
      description="Modify the details of the suppliers."
      defaultValues={record || undefined}
      open={searchQuery.action === "update" && !!searchQuery.id}
      onOpenChange={() =>
        navigate({ search: (prev) => ({ ...prev, action: undefined }) })
      }
      schema={SuppliersSchema.partial()}
      onSubmit={async (data) => {
        try {
          await pocketbase
            .collection(Collections.WarehouseManagementSuppliers)
...
export const DeleteSuppliers = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (error instanceof ClientResponseError) {
        toast.error(
          `Failed to delete suppliers: ${error.message} (${error.status})`
        );
      }
    } finally {
      navigate({ search: (prev) => ({ ...prev, action: undefined }) });
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            suppliers and remove all associated data.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel
...
export default [
  <CreateSuppliers key={"action-create"} />,
  <UpdateSuppliers key={"action-update"} />,
  <DeleteSuppliers key={"action-delete"} />,
];
