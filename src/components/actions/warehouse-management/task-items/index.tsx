...
const TaskItemsActions = () => {
	...
	if (searchQuery.action === "create") {
		Component = {
			...
			description: "Fill out the form to create a new task items.",
			Element: <CreateForm />,
		};
	}
	...
	if (searchQuery.action === "update" && searchQuery.id) {
		Component = {
			...
			description: "Modify the task items details below.",
			Element: (
        <Suspense>
          <UpdateForm />
        </Suspense>
	...
	if (searchQuery.action === "delete" && searchQuery.id) {
		Component = {
			title: "Delete Task Items",
			description: "Are you sure you want to delete this task items?",
			Element: <DeleteForm />,
		};
	}
