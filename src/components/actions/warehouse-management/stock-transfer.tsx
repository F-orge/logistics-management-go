...
export const UpdateStockTransfer = () => {
  ...
  const { data: record } = useQuery({
    queryKey: [Collections.WarehouseManagementStockTransfer, searchQuery.id],
    queryFn: async () =>
      pocketbase
        .collection(Collections.WarehouseManagementStockTransfer)
  ...
  return (
    <FormDialog
      ...
      description="Modify the details of the stock-transfer."
      defaultValues={record || undefined}
      open={searchQuery.action === "update" && !!searchQuery.id}
      onOpenChange={() =>
        navigate({ search: (prev) => ({ ...prev, action: undefined }) })
      }
      schema={StockTransferSchema.partial()}
      onSubmit={async (data) => {
        try {
          await pocketbase
            .collection(Collections.WarehouseManagementStockTransfer)
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            stock-transfer and remove all associated data.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel
...
export default [
  <CreateStockTransfer key={"action-create"} />,
  <UpdateStockTransfer key={"action-update"} />,
  <DeleteStockTransfer key={"action-delete"} />,
];
