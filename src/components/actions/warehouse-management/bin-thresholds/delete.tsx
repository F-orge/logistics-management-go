...
const DeleteBinThresholdFormDialog = () => {
	...
	const handleDelete = async () => {
		if (!searchParams.id) {
			toast.error("No Binthreshold selected");
			return;
		}
		...
		try {
			...
			if (error instanceof ClientResponseError) {
				...
				toast.error("Failed to delete Binthreshold");
			}
		} finally {
			setIsDeleting(false);
		}
	...
	return (
		<AlertDialog
			open={searchParams.action === "deleteBinThreshold"}
			onOpenChange={(open) => {
				if (!open) {
					navigate({
						search: (prev) => ({
...
export default DeleteBinThresholdFormDialog;
