...
export const CreateOpportunityProducts = () => {
  ...
  return (
    <FormDialog
      ...
      schema={OpportunityProductsSchema}
      onSubmit={async (data) => {
        try {
          await pocketbase
...
export const UpdateOpportunityProducts = () => {
  const searchQuery = useSearch({ from: "/dashboard/$schema/$collection" });
  const navigate = useNavigate({ from: "/dashboard/$schema/$collection" });
  const { pocketbase } = useRouteContext({
    from: "/dashboard/$schema/$collection",
  ...
  const { data: record } = useQuery({
    queryKey: [Collections.CustomerRelationsOpportunityProducts, searchQuery.id],
    queryFn: async () =>
      pocketbase
        .collection(Collections.CustomerRelationsOpportunityProducts)
  ...
  return (
    <FormDialog
      ...
      description="Modify the details of the opportunity-products."
      defaultValues={record || undefined}
      open={searchQuery.action === "update" && !!searchQuery.id}
      onOpenChange={() =>
        navigate({ search: (prev) => ({ ...prev, action: undefined }) })
      }
      schema={OpportunityProductsSchema.partial()}
      onSubmit={async (data) => {
        try {
          await pocketbase
            .collection(Collections.CustomerRelationsOpportunityProducts)
...
export const DeleteOpportunityProducts = () => {
  const searchQuery = useSearch({ from: "/dashboard/$schema/$collection" });
  const navigate = useNavigate({ from: "/dashboard/$schema/$collection" });
  const { pocketbase } = useRouteContext({
    from: "/dashboard/$schema/$collection",
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            opportunity-products and remove all associated data.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel
...
export default [
  <CreateOpportunityProducts key={"action-create"} />,
  <UpdateOpportunityProducts key={"action-update"} />,
  <DeleteOpportunityProducts key={"action-delete"} />,
];
