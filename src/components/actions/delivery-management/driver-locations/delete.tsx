...
const DeleteDriverLocationFormDialog = () => {
	...
	const handleDelete = async () => {
		if (!searchParams.id) {
			toast.error("No Driverlocation selected");
			return;
		}
		...
		try {
			...
			if (error instanceof ClientResponseError) {
				...
				toast.error("Failed to delete Driverlocation");
			}
		} finally {
			setIsDeleting(false);
		}
	...
	return (
		<AlertDialog
			open={searchParams.action === "deleteDriverLocation"}
			onOpenChange={(open) => {
				if (!open) {
					navigate({
						search: (prev) => ({
...
export default DeleteDriverLocationFormDialog;
