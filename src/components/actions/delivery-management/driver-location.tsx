...
export const CreateDriverLocation = () => {
...
export const UpdateDriverLocation = () => {
  ...
  const { data: record } = useQuery({
    queryKey: [Collections.DeliveryManagementDriverLocation, searchQuery.id],
    queryFn: async () =>
      pocketbase
        .collection(Collections.DeliveryManagementDriverLocation)
  ...
  return (
    <FormDialog
      ...
      description="Modify the details of the driver-location."
      defaultValues={record || undefined}
      open={searchQuery.action === "update" && !!searchQuery.id}
      onOpenChange={() =>
        navigate({ search: (prev) => ({ ...prev, action: undefined }) })
      }
      schema={DriverLocationSchema.partial()}
      onSubmit={async (data) => {
        try {
          await pocketbase
            .collection(Collections.DeliveryManagementDriverLocation)
...
export const DeleteDriverLocation = () => {
  const searchQuery = useSearch({ from: "/dashboard/$schema/$collection" });
  const navigate = useNavigate({ from: "/dashboard/$schema/$collection" });
  const { pocketbase } = useRouteContext({
    from: "/dashboard/$schema/$collection",
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            driver-location and remove all associated data.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel
...
export default [
  <CreateDriverLocation key={"action-create"} />,
  <UpdateDriverLocation key={"action-update"} />,
  <DeleteDriverLocation key={"action-delete"} />,
];
