...
type CreateSchema = z.infer<
	ReturnType<typeof CreatePartnerInvoiceItemInputSchema>
>;
type UpdateSchema = z.infer<
	ReturnType<typeof UpdatePartnerInvoiceItemInputSchema>
>;
	...
	describe("CreatePartnerInvoiceItemInputSchema", () => {
		...
		describe("SafeParse Tests", () => {
			it("should return success for valid invoice item", () => {
				...
				const result =
					CreatePartnerInvoiceItemInputSchema().safeParse(validData);
			...
			it("should return error when shipmentLegId is missing", () => {
				...
				const result =
					CreatePartnerInvoiceItemInputSchema().safeParse(invalidData);
	...
	describe("UpdatePartnerInvoiceItemInputSchema", () => {
		...
		describe("SafeParse Tests", () => {
			...
			it("should return success when amount is provided", () => {
				...
				const result =
					UpdatePartnerInvoiceItemInputSchema().safeParse(validData);
				expect(result.success).toBe(true);
			});
			...
			it("should return error when amount has wrong type", () => {
				...
				const result =
					UpdatePartnerInvoiceItemInputSchema().safeParse(invalidData);
