...
type CreateSchema = z.infer<
	ReturnType<typeof CreateOutboundShipmentInputSchema>
>;

type UpdateSchema = z.infer<
	ReturnType<typeof UpdateOutboundShipmentInputSchema>
>;
...
describe("Outbound Shipment Inputs", () => {
	describe("Create Schema", () => {
		...
		describe("Invalid Cases", () => {
			...
			it.each(cases)("should reject: $name", (testCase) => {
				const { error } = CreateOutboundShipmentInputSchema().safeParse(
					testCase.input,
				);
	...
	describe("Update Schema", () => {
		...
		describe("Invalid Cases", () => {
			...
			it.each(cases)("should reject: $name", (testCase) => {
				const { error } = UpdateOutboundShipmentInputSchema().safeParse(
					testCase.input,
				);
