...
type CreateSchema = z.infer<
	ReturnType<typeof CreateOutboundShipmentItemInputSchema>
>;

type UpdateSchema = z.infer<
	ReturnType<typeof UpdateOutboundShipmentItemInputSchema>
>;
...
describe("Outbound Shipment Item Inputs", () => {
	describe("Create Schema", () => {
		...
		describe("Invalid Cases", () => {
			...
			it.each(cases)("should reject: $name", (testCase) => {
				const { error } = CreateOutboundShipmentItemInputSchema().safeParse(
					testCase.input,
				);
	...
	describe("Update Schema", () => {
		...
		describe("Invalid Cases", () => {
			...
			it.each(cases)("should reject: $name", (testCase) => {
				const { error } = UpdateOutboundShipmentItemInputSchema().safeParse(
					testCase.input,
				);
