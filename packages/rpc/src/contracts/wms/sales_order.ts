...
import { SalesOrderItemRepository, SalesOrderRepository } from "@packages/db/repositories/wms";
...
export const PaginateSalesOrderContract = oc.input(SalesOrderRepository.schemas.paginateOptionSchema).output(OutputSchema.array());

export const RangeSalesOrderContract = oc.input(SalesOrderRepository.schemas.rangeOptionSchema).output(OutputSchema.array());

export const AnySalesOrderContract = oc.input(z.uuid().array()).output(OutputSchema.array());

export const InsertSalesOrderContract = oc.input(SalesOrderRepository.schemas.InsertSchema.extend({items:SalesOrderItemRepository.schemas.InsertSchema.array()})).output(OutputSchema);

export const InsertManySalesOrderContract = oc.input(SalesOrderRepository.schemas.InsertSchema.extend({items:SalesOrderItemRepository.schemas.InsertSchema.array()}).array()).output(OutputSchema.array());

export const UpdateSalesOrderContract = oc.input(z.object({id: z.uuid(), value: SalesOrderRepository.schemas.UpdateSchema})).output(OutputSchema);
