...
export const OutputSchema = DeliveryTaskSchema.extend({
  ...
  package: PackageSchema.extend({ items: PackageItemSchema.array() }),
...
export const AnyDeliveryTaskContract = oc
  .input(z.uuid().array())
  .output(OutputSchema.array());
...
export const UpdateDeliveryTaskContract = oc
  .input(
    z.object({
      id: z.uuid(),
      value: DeliveryTaskRepository.schemas.UpdateSchema,
    })
  )
  .output(OutputSchema);
...
export const UpdateTaskEventContract = oc
  .input(
    z.object({ id: z.uuid(), value: TaskEventRepository.schemas.UpdateSchema })
  )
  .output(OutputSchema);
