...
export const PaginateDeliveryRouteContract = oc
  .input(DeliveryRouteRepository.schemas.paginateOptionSchema)
  .output(OutputSchema.array())

export const RangeDeliveryRouteContract = oc
  .input(DeliveryRouteRepository.schemas.rangeOptionSchema)
  .output(OutputSchema.array())

export const AnyDeliveryRouteContract = oc.input(z.uuid().array()).output(OutputSchema.array())

export const InsertDeliveryRouteContract = oc
  .input(DeliveryRouteRepository.schemas.InsertSchema)
  .output(OutputSchema)

export const InsertManyDeliveryRouteContract = oc
  .input(DeliveryRouteRepository.schemas.InsertSchema.array())
  .output(OutputSchema.array())

export const UpdateDeliveryRouteContract = oc
  .input(z.object({ id: z.uuid(), value: DeliveryRouteRepository.schemas.UpdateSchema }))
  .output(OutputSchema)

export const RemoveDeliveryRouteContract = oc
  .input(z.uuid())
