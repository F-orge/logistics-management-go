...
export const AnyTripContract = oc
  .input(z.uuid().array())
  .output(OutputSchema.array());

export const InsertTripContract = oc
  .input(TripRepository.schemas.InsertSchema)
  .output(OutputSchema);
...
export const UpdateTripStopContract = oc
  .input(
    z.object({ id: z.uuid(), value: TripStopRepository.schemas.UpdateSchema })
  )
  .output(OutputSchema);
