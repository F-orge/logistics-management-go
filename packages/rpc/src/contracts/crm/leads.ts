...
export const PaginateLeadContract = oc.input(LeadRepository.schemas.paginateOptionSchema).output(OutputSchema.array());

export const RangeLeadContract = oc.input(LeadRepository.schemas.rangeOptionSchema).output(OutputSchema.array());
...
export const InsertManyLeadContract = oc.input(LeadRepository.schemas.InsertSchema.array()).output(OutputSchema.array());

export const UpdateLeadContract = oc.input(z.object({id: z.uuid(), value: LeadRepository.schemas.UpdateSchema})).output(OutputSchema);

export const RemoveLeadContract = oc.input(z.uuid()).output(z.instanceof(DeleteResult).refine((arg) => arg.numDeletedRows.toString()));
