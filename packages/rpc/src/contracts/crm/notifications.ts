...
export const PaginateNotificationContract = oc
  .input(NotificationRepository.schemas.paginateOptionSchema)
  .output(OutputSchema.array())

export const RangeNotificationContract = oc
  .input(NotificationRepository.schemas.rangeOptionSchema)
  .output(OutputSchema.array())

export const AnyNotificationContract = oc.input(z.uuid().array()).output(OutputSchema.array())

export const InsertNotificationContract = oc
  .input(NotificationRepository.schemas.InsertSchema)
  .output(OutputSchema)

export const InsertManyNotificationContract = oc
  .input(NotificationRepository.schemas.InsertSchema.array())
  .output(OutputSchema.array())

export const UpdateNotificationContract = oc
  .input(z.object({ id: z.uuid(), value: NotificationRepository.schemas.UpdateSchema }))
  .output(OutputSchema)

export const RemoveNotificationContract = oc
  .input(z.uuid())
