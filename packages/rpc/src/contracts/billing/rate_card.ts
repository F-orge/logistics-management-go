...
export const PaginateRateCardContract = oc.input(RateCardRepository.schemas.paginateOptionSchema).output(OutputSchema.array());

export const RangeRateCardContract = oc.input(RateCardRepository.schemas.rangeOptionSchema).output(OutputSchema.array());

export const AnyRateCardContract = oc.input(z.uuid().array()).output(OutputSchema.array());

export const InsertRateCardContract = oc.input(RateCardRepository.schemas.InsertSchema.extend({rules:RateRuleRepository.schemas.InsertSchema.array()})).output(OutputSchema);

export const InsertManyRateCardContract = oc.input(RateCardRepository.schemas.InsertSchema.extend({rules:RateRuleRepository.schemas.InsertSchema.array()}).array()).output(OutputSchema.array());

export const UpdateRateCardContract = oc.input(z.object({id: z.uuid(), value: RateCardRepository.schemas.UpdateSchema})).output(OutputSchema);
