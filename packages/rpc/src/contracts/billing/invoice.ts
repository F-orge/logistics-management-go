...
export const InsertInvoiceContract = oc
  .input(
    InvoiceRepository.schemas.InsertSchema.extend({
      items: InvoiceLineItemRepository.schemas.InsertSchema.array(),
    }),
  )
  .output(OutputSchema)
...
export const InsertInvoiceLineItemContract = oc
  .input(InvoiceLineItemRepository.schemas.InsertSchema)
  .output(OutputSchema)

export const InsertManyInvoiceLineItemContract = oc
  .input(InvoiceLineItemRepository.schemas.InsertSchema.array())
  .output(OutputSchema.array())

export const UpdateInvoiceLineItemContract = oc
  .input(z.object({ id: z.uuid(), value: InvoiceLineItemRepository.schemas.UpdateSchema }))
  .output(OutputSchema)

export const RemoveInvoiceLineItemContract = oc
  .input(z.uuid())
  .output(z.instanceof(DeleteResult).refine((arg) => arg.numDeletedRows.toString()))
