...
export const PaginateOpportunity = implement(contracts.PaginateOpportunityContract)
  ...
  .handler(async ({ context, input }) => {
    ...
    const campaignPromises = campaignRepo.any(result.map((row) => row.campaignId).filter(nonEmpty))
    ...
    const productPromises = productRepo.any(opportunityProducts.map((row) => row.productId))
...
export const InsertOpportunity = implement(contracts.InsertOpportunityContract)
  ...
  .handler(async ({ context, input }) => {
    ...
    const campaignPromises = campaignRepo.any([result.campaignId].filter(nonEmpty))
    const companyPromises = companyRepo.any([result.companyId].filter(nonEmpty))
...
export const InsertManyOpportunity = implement(contracts.InsertManyOpportunityContract)
  .$context<ORPCContext>()
  .handler(async ({ context, input }) => {
    const opportunityRepo = OpportunityRepository.fns(context.kysely)
    const userRepo = UserRepository.fns(context.kysely)
...
export const InsertOpportunityProduct = implement(contracts.InsertOpportunityProductContract)
  ...
  .handler(async ({ context, input }) => {
    ...
    return {
      ...result,
      // biome-ignore lint/style/noNonNullAssertion: force check since its already verified by the database
      owner: owners.find((row) => row.id === result.ownerId) as (typeof owners)[number],
      campaign: campaigns.find((row) => row.id === result.campaignId),
      company: companies.find((row) => row.id === result.companyId),
      contact: contacts.find((row) => row.id === result.contactId),
      products: opportunityProducts
        .filter((subRow) => subRow.opportunityId === result.id)
        .map(
          (subRow) => products.find((pSubRow) => pSubRow.id === subRow.productId)!,
...
export const InsertManyOpportunityProduct = implement(
  ...
  .handler(async ({ context, input }) => {
    ...
    const allOpportunityProducts = await opportunityProductRepo.paginate({
      page: 1,
      perPage: Infinity,
      filters: [{ column: 'opportunityId', operator: 'in', value: opportunityIds }],
    })
    ...
    const productPromises = productRepo.any(allOpportunityProducts.map((row) => row.productId))
    ...
    return opportunities.map((opportunity) => ({
      ...opportunity,
      // biome-ignore lint/style/noNonNullAssertion: force check since its already verified by the database
      owner: owners.find((row) => row.id === opportunity.ownerId) as (typeof owners)[number],
