...
import { CompanyRepository, OpportunityRepository } from '@packages/db/repositories/crm'
...
async function getSalesOrder(context: ORPCContext, id: string) {
  ...
  const [client, opportunity, items] = await Promise.all([
    ...
    OpportunityRepository.fns(context.kysely).find(result.opportunityId),
...
export const PaginateSalesOrder = implement(contracts.PaginateSalesOrderContract)
  .$context<ORPCContext>()
...
export const InsertManySalesOrder = implement(contracts.InsertManySalesOrderContract)
  .$context<ORPCContext>()
  .handler(async ({ context, input }) => {
    const repo = SalesOrderRepository.fns(context.kysely)
...
export const InsertSalesOrderItem = implement(contracts.InsertSalesOrderItemContract)
  .$context<ORPCContext>()
  .handler(async ({ context, input }) => {
...
export const InsertManySalesOrderItem = implement(contracts.InsertManySalesOrderItemContract)
  .$context<ORPCContext>()
  .handler(async ({ context, input }) => {
    const repo = SalesOrderItemRepository.fns(context.kysely)
...
export const UpdateSalesOrderItem = implement(contracts.UpdateSalesOrderItemContract)
  .$context<ORPCContext>()
  .handler(async ({ context, input }) => {
    const repo = SalesOrderItemRepository.fns(context.kysely)
...
export const RemoveSalesOrderItem = implement(contracts.RemoveSalesOrderItemContract)
  .$context<ORPCContext>()
  .handler(async ({ context, input }) => {
    const repo = SalesOrderItemRepository.fns(context.kysely)
