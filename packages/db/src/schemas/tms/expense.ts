...
export const ExpenseSchema = z.object({
  ...
  tripId: z.uuid({ message: 'Invalid UUID format for trip ID' }).optional().nullable(),
  type: z.enum(TmsExpenseTypeEnum, { message: 'Invalid expense type' }).optional().nullable(),
  status: z.enum(TmsExpenseStatusEnum, { message: 'Invalid expense status' }).optional().nullable(),
  ...
  currency: z.enum(TmsCurrencyEnum, { message: 'Invalid currency type' }).optional().nullable(),
  ...
  receiptUrl: z.string({ message: 'Receipt URL must be a string' }).optional().nullable(),
...
export type TmsExpense = z.infer<typeof ExpenseSchema>
