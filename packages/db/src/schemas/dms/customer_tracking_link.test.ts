...
describe('DmsCustomerTrackingLinkSchema Validation', () => {
  describe('Valid Cases', () => {
    const validTestCases:ValidCase[] = [];

    test.each(validTestCases)('should validate: $name', ({ input }) => {
      expect(() => CustomerTrackingLinkSchema.parse(input)).not.toThrow();
      const result = CustomerTrackingLinkSchema.parse(input);
  ...
  describe('Invalid Cases', () => {
    ...
    test.each(invalidTestCases)(
      ...
      ({ input, expectedError }) => {
        ...
        try {
          CustomerTrackingLinkSchema.parse(input);
        } catch (e) {
